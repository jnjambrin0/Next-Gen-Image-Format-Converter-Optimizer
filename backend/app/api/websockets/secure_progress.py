...
from fastapi import WebSocket, WebSocketDisconnect, WebSocketException, status, Query, HTTPException
...
class SecureConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, job_id: str, token: Optional[str] = None, client_ip: Optional[str] = None) -> bool:
...
async def websocket_endpoint(
    ...
    if not job_id or len(job_id) != 36:  # UUID format
        await websocket.close(code=status.WS_1003_UNSUPPORTED_DATA, reason="Invalid job ID format")
        return
        ...
        while True:
            try:
                ...
                try:
                    ...
                    await secure_connection_manager.handle_client_message(websocket, message)
