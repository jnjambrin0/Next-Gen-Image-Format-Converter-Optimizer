...
async def toggle_paranoia_mode(enable: bool = Query(description="Enable or disable paranoia mode")):
    """
    ...
    try:
        ...
        setup_logging(
            log_level=settings.log_level,
            json_logs=True,
            enable_file_logging=not enable
        )
        ...
        return {
            ...
            "data": {
                ...
                "logging_enabled": not enable,
                "message": "Paranoia mode enabled - all file logging disabled" if enable 
...
async def cleanup_old_stats(
    hourly_retention: int = Query(default=168, ge=24, le=720, description="Hours to retain hourly stats"),
    daily_retention: int = Query(default=90, ge=7, le=365, description="Days to retain daily stats")
):
    """
...
async def cleanup_old_errors(
    retention_days: int = Query(default=30, ge=1, le=90, description="Days to retain errors")
):
    """
...
async def get_violation_trends(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve violation trends")


@router.post("/security/cleanup", response_model=Dict[str, Any])
async def cleanup_security_events(
    retention_days: int = Query(default=90, ge=7, le=365, description="Days to retain events")
):
    """
