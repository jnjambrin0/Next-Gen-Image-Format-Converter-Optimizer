...
def validate_batch_request(
    ...
    if output_format not in valid_output_formats:
        raise batch_create_error_handler.validation_error(
            ...
            request,
            details={"requested_format": output_format, "valid_formats": valid_output_formats}
        )
    ...
    for i, file in enumerate(files):
        ...
        if ext not in valid_extensions:
            raise batch_create_error_handler.validation_error(
                ...
                request,
                details={"file_index": i, "extension": ext, "supported_extensions": valid_extensions}
            )
            ...
            try:
                ...
                if status_filter not in valid_statuses:
                    raise batch_status_error_handler.validation_error(
                        ...
                        request,
                        details={"provided_status": status_filter, "valid_statuses": valid_statuses}
                    )
                
                # Filter items by status
                filtered_items = [
                    item for item in batch_job.items 
    ...
    try:
        ...
        if batch_job.status in [BatchStatus.COMPLETED, BatchStatus.FAILED]:
            raise batch_cancel_error_handler.validation_error(
                ...
                request,
                details={"current_status": batch_job.status.value, "cancellable_statuses": ["pending", "processing"]}
            )
        ...
        cancelled_items = 0
        if updated_job:
            cancelled_items = sum(1 for item in updated_job.items if item.status == BatchItemStatus.CANCELLED)
...
async def cancel_batch_item(job_id: str, file_index: int, request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        if format == "json":
            ...
            summary = {
                ...
                "completed_files": batch_job.completed_files,
                "failed_files": batch_job.failed_files,
                "processing_time": (batch_job.completed_at - batch_job.created_at).total_seconds() if batch_job.completed_at else None,
                "files": [
                    {
                        "filename": item.filename,
                        "status": item.status.value,
                        "error": item.error if item.status == BatchItemStatus.FAILED else None
                    }
    ...
    try:
        ...
        if not metrics:
            raise batch_metrics_error_handler.not_found_error(
                "Metrics not found for this job",
                request,
                details={"job_id": job_id}
            )
        ...
        raise batch_metrics_error_handler.internal_server_error(
            "Failed to retrieve batch metrics",
            request
        )
...
async def create_websocket_token(job_id: str, request: Request) -> Dict[str, str]:
    ...
    try:
        ...
        raise batch_token_error_handler.internal_server_error(
            "Failed to create WebSocket token",
            request
        )
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        if (current_job.progress_percentage != last_progress_percentage or 
                        ...
                        if current_job.status in [BatchStatus.COMPLETED, BatchStatus.FAILED, BatchStatus.CANCELLED]:
                            logger.info(f"Job reached terminal state: {current_job.status.value}")
                            break
