...
async def get_network_status(
    ...
    if hasattr(request.app.state, "network_status"):
        ...
        response["verification_status"] = (
            "verified" if network_status.get("verified", False) else "not_verified"
        )
    ...
    if tracker:
        try:
            ...
            response["violation_trends"] = {
                ...
                "trend": trends["trend"],
                "daily_average": (
                    sum(d["total"] for d in trends["daily_violations"])
                    / len(trends["daily_violations"])
                    if trends["daily_violations"]
                    else 0
                ),
            }
...
async def get_security_events(
    ...
    if not tracker:
        return {"events": [], "message": "Security event tracking not available"}
...
async def verify_network_isolation(
    request: Request, strictness: Optional[str] = None
) -> Dict[str, Any]:
    """
