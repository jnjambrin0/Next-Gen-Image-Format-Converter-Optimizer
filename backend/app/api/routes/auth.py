...
class ApiKeyCreateRequest(BaseModel):
    ...
    rate_limit_override: Optional[int] = Field(None, ge=1, le=1000, description="Custom rate limit per minute (1-1000)")
...
class ApiKeyUpdateRequest(BaseModel):
    ...
    name: Optional[str] = Field(None, max_length=100, description="New name for the API key")
    rate_limit_override: Optional[int] = Field(None, ge=1, le=1000, description="New rate limit per minute")
    expires_days: Optional[int] = Field(None, ge=1, le=365, description="New expiration in days from now")
...
async def list_api_keys(
    ...
    try:
        ...
        logger.info(
            ...
            client_ip=getattr(http_request.client, 'host', 'unknown') if http_request and http_request.client else 'unknown'
        )
