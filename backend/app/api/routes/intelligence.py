...
from app.models.conversion import ContentType, ContentClassification, OutputFormat, InputFormat
...
async def get_format_recommendations(
    content_classification: ContentClassification = Body(..., description="Content classification from analysis"),
    original_format: InputFormat = Body(..., description="Original image format"),
    original_size_kb: int = Body(..., gt=0, le=102400, description="Original file size in KB (max 100MB)"),
    use_case: Optional[UseCaseType] = Body(None, description="Intended use case"),
    prioritize: Optional[str] = Body(None, regex="^(size|quality|compatibility)$", description="What to prioritize: size/quality/compatibility"),
    exclude_formats: Optional[List[OutputFormat]] = Body(None, description="Formats to exclude"),
    override_format: Optional[OutputFormat] = Body(None, description="User override format choice")
) -> JSONResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to generate format recommendations"
        )
...
async def record_format_preference(
    ...
    try:
        ...
        return JSONResponse(
            content={"message": "Preference recorded successfully"},
            status_code=200
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to record preference"
        )
...
async def get_format_preferences(
    content_type: ContentType,
    use_case: Optional[UseCaseType] = Query(None, description="Optional use case filter")
) -> JSONResponse:
...
async def reset_format_preferences(
    content_type: Optional[ContentType] = Body(None, description="Content type to reset"),
    ...
    try:
        ...
        return JSONResponse(
            content={
                "message": f"Reset {count} preferences",
                "count": count
            },
            status_code=200
        )
...
async def get_format_details(
    ...
    """
    try:
        details = await recommendation_service.get_format_details(
            format,
            content_type
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to get format details"
        )