...
from app.models.optimization import (OptimizationMode, OptimizationRequest,
...
async def optimize_advanced(
    ...
    chroma_subsampling: Optional[str] = Form(
    ...
    try:
        ...
        if not detected_format:
            raise HTTPException(
                status_code=400, detail="Unsupported or corrupted file format"
            )
        ...
        try:
            response = await asyncio.wait_for(
                optimization_service.optimize_image(
                    file_data, request, detected_format
                ),
            ...
            logger.warning(
                f"Optimization timeout after {OPTIMIZATION_TIMEOUT_SECONDS}s"
            )
            raise HTTPException(
                status_code=504,
        ...
        raise HTTPException(
            status_code=500, detail=error_info.get("message", "Internal server error")
        )
...
async def optimize_advanced_download(
    ...
    try:
        ...
        if not response.success or not optimized_data:
            raise HTTPException(
                status_code=500, detail=response.error_message or "Optimization failed"
            )
...
async def get_format_capabilities(format: str):
    ...
    encoding_caps = optimization_service.encoding_options.get_format_capabilities(
        format
    )
    ...
    lossless_caps = optimization_service.lossless_compressor.get_format_capabilities(
        format
    )
