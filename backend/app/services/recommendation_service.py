...
class RecommendationService:
    ...
    def __init__(self, preference_db_path: Optional[str] = None) -> None:
        ...
        self.preference_tracker = UserPreferenceTracker(db_path=db_path)
    ...
    async def get_recommendations(
        ...
        if not request.content_classification:
            raise create_verification_error(
                "missing_classification", "Content classification required"
            )
        ...
        try:
            ...
            if override_format:
                ...
                recommendations = [
                    await self._create_override_recommendation(override_format, request)
                ]
            else:
            ...
            comparison_matrix = self._create_comparison_matrix(recommendations, request)
            ...
            raise create_file_error(
                "invalid_input", "Invalid recommendation request data"
            )
        except asyncio.TimeoutError:
            logger.error("Recommendation generation timed out")
            raise create_rate_limit_error(
                "timeout", "Recommendation generation timed out"
            )
        except Exception as e:
            logger.error("Unexpected error generating recommendations")
            raise create_file_error(
                "processing_failed", "Failed to generate recommendations"
            )
    ...
    async def _create_override_recommendation(
        ...
        compatibility = self.format_analyzer.analyze_format_compatibility(
            request.original_format, format_enum
        )
    ...
    async def record_user_choice(
        ...
        try:
            await self.preference_tracker.record_preference(
                content_type, chosen_format, use_case
            )
    ...
    async def get_user_preferences(
        self, content_type: ContentType, use_case: Optional[UseCaseType] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_format_details(
        ...
        content_score = RecommendationEngine.CONTENT_SCORES.get(content_type, {}).get(
            format_enum, 0.5
        )
