...
class ApiKeyService:
    ...
    def update_api_key(
        ...
        with self.SessionLocal() as session:
            ...
            if expires_days is not None:
                update_values['expires_at'] = datetime.utcnow() + timedelta(days=expires_days)
    ...
    def get_usage_stats(
        ...
        with self.SessionLocal() as session:
            query = select(ApiUsageStats).where(
                ApiUsageStats.timestamp >= since
            )
            ...
            avg_response_time = sum(stat.response_time_ms for stat in stats) / max(total_requests, 1)
            
            status_codes = {}
            endpoints = {}
            for stat in stats:
                status_codes[stat.status_code] = status_codes.get(stat.status_code, 0) + 1
