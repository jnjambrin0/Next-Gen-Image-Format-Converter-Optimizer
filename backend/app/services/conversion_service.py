...
class ConversionService:
    ...
    async def convert(
        ...
        try:
            ...
            try:
                detected_format, is_confident = (
                    await format_detection_service.detect_format(image_data)
                )
                logger.info(
                    "Format detected",
                    ...
                    if preset:
                        ...
                        if conversion_settings:
                            ...
                            if preset.settings.resize_options:
                                preset_dict["resize_options"] = (
                                    preset.settings.resize_options
                                )
                            if preset.settings.advanced_settings:
                                preset_dict["advanced_settings"] = (
                                    preset.settings.advanced_settings
                                )

                            # Override with request settings
                            request_dict = conversion_settings.model_dump(
                                exclude_none=True
                            )
                        ...
                        logger.warning("Preset not found", preset_id=request.preset_id)
                except Exception as e:
                    logger.error(
    ...
    async def convert_with_advanced_options(
        ...
        **advanced_options,
        ...
        core_request = CoreConversionRequest(
            output_format=output_format, settings=settings
        )

        # Perform conversion
        result = await self.conversion_manager.convert_image(
            image_data, detected_format, core_request
        )
    ...
    async def validate_image(self, image_data: bytes, format_hint: Optional[str] = None) -> bool:
        ...
        Args:
            image_data: Raw image data
            format_hint: Optional expected format for validation
        ...
        # HEIC files can have variable box sizes, check multiple positions
            ...
            # Some HEIC files have different structure, check for 'ftyp' in first 40 bytes
                ...
                if ftyp_index >= 0 and len(data) > ftyp_index + 8:
                    ...
                    for i in range(
                        ftyp_index + 8, min(ftyp_index + 40, len(data) - 4), 4
                    ):
