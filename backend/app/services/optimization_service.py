...
class OptimizationService:
    ...
    def __init__(self):
        ...
        self.optimization_engine = OptimizationEngine(quality_analyzer=self.quality_analyzer)
    ...
    async def optimize_image(
        ...
        try:
            ...
            alpha_info = None
            if request.alpha_quality is not None or request.output_format.lower() in ['webp', 'png']:
                optimized_data, alpha_info_dict = await self.alpha_optimizer.optimize_alpha(
                    image_data,
                    request.output_format,
                    request.alpha_quality,
            ...
            if request.multi_pass:
                ...
                compressed_data, compression_info = await self.lossless_compressor.compress_lossless(
                    image_data,
                    request.output_format,
            ...
            if request.perceptual_metrics:
                ...
                size_reduction = await self.quality_analyzer.calculate_file_size_reduction(
                    original_size,
                    len(image_data)
                )
    ...
    async def analyze_optimization_potential(
        ...
        try:
            ...
            alpha_analysis = await self.alpha_optimizer.analyze_alpha_channel(image_data)
            ...
            lossless_caps = self.lossless_compressor.get_format_capabilities(output_format)
            ...
            if format_caps.get('progressive') and output_format.lower() in ['jpeg', 'png']:
            ...
            if lossless_caps.get('native') and output_format.lower() != 'jpeg':
                recommendations.append("Consider lossless compression for quality preservation")
