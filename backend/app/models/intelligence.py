...
class ContentClassification(BaseModel):
    ...
    face_regions: List[BoundingBox] = Field(
        ...
        description="Detected face regions"
    )
    ...
    has_ui_elements: bool = Field(False, description="Whether UI elements were detected")
    ...
    color_complexity: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Color complexity score"
    )
    ...
    noise_level: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Noise level in the image"
    )
...
class OptimizationRecommendation(BaseModel):
    ...
    recommended_quality: int = Field(..., ge=1, le=100, description="Recommended quality")
    
    # Format-specific recommendations
    use_lossless: bool = Field(False, description="Whether to use lossless compression")
    use_progressive: bool = Field(False, description="Whether to use progressive encoding")
    
    # Optimization settings
    preserve_transparency: bool = Field(False, description="Whether to preserve transparency")
    optimize_for: str = Field("balanced", description="Optimization target: 'size', 'quality', 'balanced'")
    ...
    estimated_quality_loss: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Estimated quality loss percentage"
    )
...
class ModelInfo(BaseModel):
    ...
    # Capabilities
    supports_gpu: bool = Field(False, description="Whether GPU acceleration is supported")
    supports_batch: bool = Field(False, description="Whether batch processing is supported")
...
class IntelligenceCapabilities(BaseModel):
    ...
    content_detection: bool = Field(True, description="Content type detection available")
    ...
    sentiment_analysis: bool = Field(False, description="Image sentiment analysis available")
