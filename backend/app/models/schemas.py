...
class PresetSettings(BaseModel):
    ...
    optimization_mode: str = Field(
        "balanced", description="Optimization mode: balanced, file_size, or quality"
    )
    preserve_metadata: bool = Field(
        False, description="Whether to preserve image metadata"
    )
    resize_options: Optional[Dict[str, Any]] = Field(
        None, description="Optional resize configuration"
    )
    advanced_settings: Optional[Dict[str, Any]] = Field(
        None, description="Format-specific advanced settings"
    )
    ...
    def validate_optimization_mode(cls, v: str) -> str:
        ...
        if v not in valid_modes:
            raise ValueError(
                f"Invalid optimization mode: {v}. Must be one of {valid_modes}"
            )
        return v
...
class PresetBase(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=500, description="Preset description"
    )
    settings: PresetSettings = Field(
        ..., description="Conversion settings for this preset"
    )
