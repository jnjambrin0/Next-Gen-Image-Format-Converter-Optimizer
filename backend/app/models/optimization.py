...
class OptimizationRequest(BaseModel):
    ...
    multi_pass: bool = Field(default=False, description="Enable multi-pass optimization")
    ...
    region_optimization: bool = Field(
        default=False,
        description="Enable region-based optimization"
    )
    perceptual_metrics: bool = Field(
        default=True,
        description="Calculate perceptual quality metrics"
    )
    
    # Advanced encoding options
    chroma_subsampling: Optional[ChromaSubsampling] = Field(
        None,
        description="Chroma subsampling mode"
    )
    ...
    alpha_quality: Optional[int] = Field(
        ...
        ge=1,
        le=100,
        description="Alpha channel quality (1-100)"
    )
    custom_quantization: Optional[Dict[str, Any]] = Field(
        None,
        description="Custom quantization tables"
    )
    ...
    min_quality: int = Field(
        ...
        ge=1,
        le=100,
        description="Minimum quality for optimization"
    )
    max_quality: int = Field(
        default=95,
        ge=1,
        le=100,
        description="Maximum quality for optimization"
    )
    base_quality: int = Field(
        default=85,
        ge=1,
        le=100,
        description="Base quality setting"
    )
...
class QualityMetrics(BaseModel):
    ...
    file_size_reduction: float = Field(
        ...
        ge=0.0,
        le=100.0,
        description="File size reduction percentage"
    )
...
class OptimizationPass(BaseModel):
    ...
    processing_time: float = Field(..., ge=0.0, description="Pass processing time in seconds")
...
class AlphaChannelInfo(BaseModel):
    ...
    alpha_usage: Literal["none", "unnecessary", "binary", "mostly_binary", "simple", "complex"]
...
class OptimizationResponse(BaseModel):
    ...
    total_passes: Optional[int] = Field(None, description="Number of optimization passes")
    converged: Optional[bool] = Field(None, description="Whether optimization converged")
    passes: Optional[List[OptimizationPass]] = Field(None, description="Details of each pass")
...
class OptimizationProgressUpdate(BaseModel):
    ...
    conversion_id: UUID
    status: Literal["started", "analyzing", "optimizing", "finalizing", "completed", "failed"]
