...
class FormatCharacteristics(BaseModel):
    ...
    compression_efficiency: float = Field(
        ge=0.0, le=1.0, description="Compression efficiency (0-1, higher is better)"
    )
    browser_support: float = Field(
        ge=0.0, le=1.0, description="Browser/device support level (0-1)"
    )
    quality_preservation: float = Field(
        ge=0.0, le=1.0, description="Quality preservation capability (0-1)"
    )
    ...
    processing_speed: float = Field(
        ge=0.0, le=1.0, description="Encoding/decoding speed (0-1, higher is faster)"
    )
    future_proof: float = Field(
        ge=0.0, le=1.0, description="Future compatibility likelihood (0-1)"
    )
...
class TradeOffAnalysis(BaseModel):
    ...
    size_reduction: float = Field(
        ge=0.0, le=1.0, description="Expected size reduction (0-1, higher is better)"
    )
    quality_score: float = Field(
        ge=0.0, le=1.0, description="Expected quality score (0-1)"
    )
    compatibility_score: float = Field(
        ge=0.0, le=1.0, description="Compatibility score (0-1)"
    )
    feature_score: float = Field(
        ge=0.0, le=1.0, description="Feature support score (0-1)"
    )
    performance_score: float = Field(
        ge=0.0, le=1.0, description="Processing performance score (0-1)"
    )
...
class FormatRecommendation(BaseModel):
    ...
    score: float = Field(
        ge=0.0, le=1.0, description="Overall recommendation score (0-1)"
    )
    reasons: List[str] = Field(
        default_factory=list, description="Human-readable reasons for recommendation"
    )
    estimated_size_kb: int = Field(gt=0, description="Estimated output file size in KB")
    quality_score: float = Field(
        ge=0.0, le=1.0, description="Predicted quality score (0-1)"
    )
    compatibility_score: float = Field(
        ge=0.0, le=1.0, description="Browser/device compatibility score (0-1)"
    )
    features: Dict[str, bool] = Field(
        default_factory=dict, description="Supported features for this format"
    )
    trade_offs: TradeOffAnalysis = Field(description="Detailed trade-off analysis")
    pros: List[str] = Field(
        default_factory=list, description="Advantages of this format for the use case"
    )
    cons: List[str] = Field(
        default_factory=list, description="Disadvantages of this format"
    )
...
class RecommendationRequest(BaseModel):
    ...
    use_case: Optional[UseCaseType] = Field(
        None, description="Intended use case (web/print/archive)"
    )
    original_format: InputFormat = Field(description="Original image format")
    original_size_kb: int = Field(gt=0, description="Original file size in KB")
    prioritize: Optional[str] = Field(
        None, description="What to prioritize: size/quality/compatibility"
    )
    exclude_formats: Optional[List[OutputFormat]] = Field(
        default_factory=list, description="Formats to exclude from recommendations"
    )
...
class RecommendationResponse(BaseModel):
    ...
    comparison_matrix: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Comparison matrix for recommended formats"
    )
    content_type: str = Field(description="Detected content type")
    use_case: Optional[str] = Field(None, description="Applied use case")
...
class FormatComparisonMetric(BaseModel):
    ...
    metric_value: float = Field(
        ge=0.0, le=1.0, description="Normalized metric value (0-1)"
    )
