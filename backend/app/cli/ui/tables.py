...
class SmartTable:
    ...
    def render(self, max_rows: Optional[int] = None) -> Table:
        ...
        for col_idx, col in enumerate(self.columns):
            ...
            table.add_column(
                header, justify=col["justify"], width=col_width, overflow="ellipsis"
            )
...
def create_conversion_table(
    ...
    table = SmartTable(
        title="Conversion Results", console=console, show_statistics=True
    )
...
def create_batch_summary_table(
    batch_results: List[Dict[str, Any]], console: Optional[Console] = None
) -> SmartTable:
    """Create a batch operation summary table"""
    table = SmartTable(
        title="Batch Operation Summary", console=console, show_statistics=True
    )
