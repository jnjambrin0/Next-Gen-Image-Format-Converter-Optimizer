...
from textual.widgets import (
    Button, Static, Input, Label, Select, 
    ListView, ListItem, ProgressBar, Switch,
    RadioButton, RadioSet, Tree
)
...
class PresetPicker(Container):
    ...
    PRESETS = {
        ...
        "print": {"name": "Print", "icon": "printer", "description": "High quality for printing"},
        "archive": {"name": "Archive", "icon": "archive", "description": "Archival quality"},
        "thumbnail": {"name": "Thumbnail", "icon": "image", "description": "Small thumbnails"},
        "social": {"name": "Social Media", "icon": "share", "description": "Social media optimized"},
    }
    ...
    def _update_list(self) -> None:
        ...
        for file_path in self.files:
            list_view.append(
                ListItem(
                    Static(
                        f"{get_format_emoji(file_path.suffix[1:])} {file_path.name}"
                    )
                )
            )
...
class ConversionForm(Container):
    ...
    def get_options(self) -> ConversionOptions:
        ...
        self.options.preserve_metadata = self.query_one("#preserve_metadata", Switch).value
...
class BatchProgressDisplay(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        # Overall progress
        yield Static(
            f"{get_emoji('batch')} Overall: 0/{self.total_files}",
            id="overall_label"
        )
        yield ProgressBar(
        ...
        yield Static(
            f"{get_emoji('file')} Current File: -",
            id="current_file_label"
        )
        yield ProgressBar(
            total=100,
            id="current_file_progress"
        )
    ...
    def _update_progress(self) -> None:
        ...
        if self.completed_files == 0:
            ...
            status.set_status(
                "processing",
                f"Processing... {self.completed_files}/{self.total_files}"
            )
        else:
