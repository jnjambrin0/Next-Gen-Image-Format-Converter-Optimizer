...
from textual.widgets import (
    ...
    Select, DirectoryTree, ListView, ListItem,
    ProgressBar, DataTable, RichLog, Tabs, Tab,
    RadioButton, RadioSet, Checkbox, Switch
)
...
class ConversionSettings(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Select(
            [(f"{get_format_emoji(fmt)} {fmt.upper()}", fmt) for fmt in 
             ["webp", "avif", "jpeg", "png", "jxl", "heif"]],
            id="output_format",
            value="webp"
        )
        ...
        yield Select(
            [("None", None), ("Web", "web"), ("Print", "print"), 
             ("Archive", "archive"), ("Thumbnail", "thumbnail")],
            id="preset",
            value=None
        )
...
class ImageConverterTUI(App):
    ...
    def __init__(self):
        ...
        self.progress_limiter = RateLimiter(min_interval=0.1)  # 10 updates per second max
        self.log_limiter = RateLimiter(min_interval=0.05)      # 20 log entries per second max
        self.table_limiter = RateLimiter(min_interval=0.2)     # 5 table updates per second max
    ...
    def on_mount(self) -> None:
        ...
        tree.filter = lambda path: (
            path.is_dir() or 
            path.suffix.lower() in {'.jpg', '.jpeg', '.png', '.gif', 
                                   '.webp', '.avif', '.heif', '.heic',
                                   '.bmp', '.tiff', '.tif'}
        )
    ...
    async def action_convert(self) -> None:
        ...
        if not SDK_AVAILABLE:
            self._log_message("SDK not available. Please install the Image Converter SDK.", "error")
            return
        ...
        if not quality_input.is_valid:
            self._log_message("Invalid quality value. Must be between 1 and 100.", "error")
        ...
        self._log_message(f"Starting conversion of {len(self.selected_files)} files to {output_format.upper()}", "info")
        ...
        for i, file_path in enumerate(self.selected_files):
            ...
            try:
                ...
                output_path = file_path.parent / f"{sanitized_name}.{output_format.lower()}"
                
                # Validate output path
                if not self.path_sanitizer.is_safe_path(output_path.parent, self.base_path):
                ...
                savings = int((1 - converted_size / original_size) * 100) if original_size > 0 else 0
                ...
                self._log_message(f"Converted {file_path.name} - Saved {savings}%", "success")
                ...
                self._log_message(f"Failed to convert {file_path.name}: {str(e)}", "error")
    ...
    def action_toggle_dark(self) -> None:
        ...
        self._log_message(f"Dark mode: {'enabled' if self.dark else 'disabled'}", "info")
