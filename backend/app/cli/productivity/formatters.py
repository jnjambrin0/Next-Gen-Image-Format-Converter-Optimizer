...
class OutputFormatter:
    ...
    def format_json(data: Any, pretty: bool = True, indent: int = 2) -> str:
        ...
        if pretty:
            return json.dumps(data, indent=indent, default=json_serializer, sort_keys=True)
        else:
    ...
    def format_json_jq(data: Any) -> str:
        ...
        if isinstance(data, list):
            ...
            lines = []
            for item in data:
                lines.append(json.dumps(item, default=json_serializer, separators=(',', ':')))
            return '\n'.join(lines)
        else:
            return json.dumps(data, default=json_serializer, separators=(',', ':'))
    ...
    def format_table(data: Any, fields: Optional[List[str]] = None) -> str:
        ...
        for row in flat_data:
            row_str = " | ".join(
                str(row.get(field, "")).ljust(col_widths[field])
                for field in fields
            )
    ...
    def format_markdown(data: Any, fields: Optional[List[str]] = None) -> str:
        ...
        if isinstance(data, dict):
            ...
            lines.append("| " + " | ".join(str(flat_dict[f]) for f in table_fields) + " |")
...
class ErrorFormatter:
    ...
    def format_batch_errors(
        errors: List[Dict],
        format: Union[OutputFormat, str] = OutputFormat.CSV
    ) -> str:
        """Format batch errors"""
    ...
    def format_summary(
        ...
        if format == OutputFormat.MARKDOWN or format == "markdown":
            ...
            if "successful" in summary:
                success_rate = (summary['successful'] / summary['total_files'] * 100) if summary.get('total_files') else 0
                lines.append(f"**Success Rate**: {success_rate:.1f}%\n")
            if "average_compression" in summary:
                lines.append(f"**Average Compression**: {summary['average_compression'] * 100:.0f}%\n")
            ...
            if "top_compressions" in summary:
                ...
                for item in summary['top_compressions']:
                    lines.append(f"- **{item['file']}**: {item['ratio'] * 100:.0f}% reduction")
    ...
    def format_batch_result(
        ...
        result_data = {
            ...
            "metrics": {
                ...
                "avg_time_per_file": round(total_time / total_files, 3) if total_files > 0 else 0,
                "total_input_bytes": total_input_size,
                "total_output_bytes": total_output_size,
                "total_reduction_percent": round((1 - total_output_size / total_input_size) * 100, 2) if total_input_size > 0 else 0,
            },
            "timestamp": datetime.now().isoformat(),
        }
...
class TableFormatter:
        ...
        if style == "markdown":
            ...
            for row in data:
                if max_rows and row_count >= max_rows:
                    lines.append("|" + "|".join(" ... ".center(widths[f] + 2) for f in fields) + "|")
                    lines.append(f"... {len(data) - max_rows} more rows truncated ...")
                    break
                    
                row_str = "|" + "|".join(f" {str(row.get(f, '')).ljust(widths[f])} " for f in fields) + "|"
                lines.append(row_str)
                row_count += 1
    ...
    @staticmethod
    def calculate_column_widths(data: List[Dict], fields: Optional[List[str]] = None) -> Dict[str, int]:
