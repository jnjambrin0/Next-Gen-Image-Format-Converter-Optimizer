...
from app.cli.config import get_config_dir, CLIConfig, get_config
...
class Profile:
    ...
    def get_effective_settings(self, parent_profile: Optional['Profile'] = None) -> Dict[str, Any]:
        """
...
class ProfileManager:
    ...
    def import_profile(self, input_file: Path, rename: Optional[str] = None) -> Optional[Profile]:
        """
        ...
        try:
            ...
            if profile.name in self.BUILTIN_PROFILES or profile.name in self.user_profiles:
                # Auto-rename with timestamp
                profile.name = f"{profile.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def clone_profile(self, source_name: str, new_name: str, new_description: Optional[str] = None) -> Profile:
        """
