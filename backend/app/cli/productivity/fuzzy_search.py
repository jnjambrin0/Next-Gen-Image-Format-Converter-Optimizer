...
class FuzzySearcher:
    ...
    def search(
        ...
        partial_results = process.extract(
            query, commands, scorer=fuzz.partial_ratio, limit=limit * 2
        )

        # Strategy 3: Levenshtein distance for typos
        lev_results = process.extract(
            query, commands, scorer=fuzz.ratio, limit=limit * 2
        )
        ...
        for cmd, score, _ in token_results:
            if score >= self.threshold:
                score_map[cmd] = (
                    score_map.get(cmd, 0) + score * 1.2
                )  # Weight token matches higher
    ...
    def search_with_filters(
        ...
        if time_range:
            ...
            filtered_history = [
                e for e in filtered_history if start <= e.timestamp <= end
            ]

        # Apply command prefix filter
        if command_prefix:
            filtered_history = [
                e for e in filtered_history if e.command.startswith(command_prefix)
            ]

        return self.search(query, filtered_history, limit)

    def find_similar_commands(
        self, command: str, history: List[HistoryEntry], limit: int = 5
    ) -> List[Tuple[HistoryEntry, float]]:
        """
    ...
    def search_by_pattern(
        self, pattern: str, history: List[HistoryEntry], is_regex: bool = False
    ) -> List[HistoryEntry]:
        """
    ...
    def get_command_frequency(
        self, history: List[HistoryEntry], top_n: int = 10
    ) -> List[Tuple[str, int]]:
        """
    ...
    def get_command_patterns(self, history: List[HistoryEntry]) -> Dict[str, List[str]]:
        """
...
class InteractiveHistoryBrowser:
    ...
    def search_and_display(
        self, query: str, history: List[HistoryEntry], display_callback=None
...
class HistoryExporter:
    ...
    def export_sanitized(
        history: List[HistoryEntry], output_file: str, remove_pii: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            for entry in history:
                ...
                if remove_pii:
                    # Sanitize command
                    entry_dict["command"] = PrivacySanitizer.sanitize(
                        entry_dict["command"]
                    )
    ...
    @staticmethod
    def import_history(
        input_file: str, validate: bool = True
