...
class CommandLearner:
    ...
    def _load_patterns(self) -> Dict:
        ...
        try:
            with open(self.data_file, 'rb') as f:
                ...
                data["parameters"] = defaultdict(Counter, {
                    k: Counter(v) for k, v in data["parameters"].items()
                })
                data["sequences"] = defaultdict(Counter, {
                    k: Counter(v) for k, v in data["sequences"].items()
                })
                data["contexts"] = defaultdict(Counter, {
                    k: Counter(v) for k, v in data["contexts"].items()
                })
    ...
    def learn(self, command: str, context: Optional[str] = None, previous_command: Optional[str] = None):
        """
    ...
    def get_suggestions(self, partial: str, context: Optional[str] = None, limit: int = 5) -> List[str]:
        """
...
class AutocompleteEngine:
    ...
    def _build_command_registry(self) -> Dict[str, Dict]:
        ...
        return {
            "convert": {
                "params": ["-f", "--format", "-o", "--output", "-q", "--quality", 
                          "--preset", "--preserve-metadata", "--strip-metadata"],
                "requires_file": True,
                "description": "Convert image to different format"
            },
            "batch": {
                "params": ["-f", "--format", "-q", "--quality", "--preset", 
                          "--workers", "--progress"],
                "requires_file": True,
                "description": "Batch convert multiple images"
            },
            "optimize": {
                "params": ["--preset", "--target-size", "--max-width", "--max-height",
                          "--quality", "--lossless"],
                "requires_file": True,
            ...
            "presets": {
                "params": ["--list", "--create", "--edit", "--delete", "--export", "--import"],
                "requires_file": False,
            ...
            "watch": {
                "params": ["--filter", "--exclude", "--format", "--preset", "--workers"],
                "requires_file": False,
    ...
    def get_suggestions(self, partial_input: str, context: Optional[str] = None) -> List[Tuple[str, str]]:
        """
