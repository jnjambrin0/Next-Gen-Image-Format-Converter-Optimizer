...
class ReferenceCardGenerator:
    ...
    def _generate_text_fallback(self, card_type: str = "basic", output_path: Optional[Path] = None) -> Optional[Path]:
        """
        ...
        lines.append("=" * 70)
        lines.append("Note: PDF generation unavailable. Install 'reportlab' for PDF support.")
        lines.append("=" * 70)
        
        # Write to file
        output_path.write_text("\n".join(lines))
        
        self.console.print(f"[green]✓[/green] Text reference card saved to: {output_path}")
        return output_path
    
    def generate_pdf(self, card_type: str = "basic", output_path: Optional[Path] = None) -> Optional[Path]:
        """
        ...
        if not REPORTLAB_AVAILABLE:
            self.console.print("[yellow]PDF generation not available. Falling back to text format.[/yellow]")
        ...
        story.append(Spacer(1, 0.3*inch))
        story.append(Paragraph(
            "Image Converter CLI - Professional Image Processing",
            styles['Italic']
        ))
        ...
        self.console.print(f"[green]✓[/green] Generated PDF reference card: {output_path}")
        return output_path
    ...
    def display_card(self, card_type: str = "basic"):
        ...
        self.console.print(f"[dim]Generated: {datetime.now().strftime('%Y-%m-%d')}[/dim]\n")
