...
class HelpContextAnalyzer:
    ...
    def __init__(self, console: Optional[Console] = None, cache_ttl: int = DEFAULT_CACHE_TTL):
    ...
    def get_context(self, ctx: typer.Context) -> HelpContext:
        ...
        # Generate cache key
        cache_key = f"{':'.join(command_chain)}:{str(current_params)}:{str(error_state)}"
        ...
        if command_chain:
            ...
            if primary_command in self.help_topics:
                ...
                if error_state:
                    ...
                    import re
                    error_code_match = re.search(r'(CONV|BATCH|OPT)\d{3}', str(error_state))
                    if error_code_match:
    ...
    def search_help(self, query: str) -> List[Dict[str, Any]]:
        ...
        for command, topic in self.help_topics.items():
            ...
            for error_code, error_msg in topic.get("common_errors", {}).items():
                if query_lower in error_code.lower() or query_lower in error_msg.lower():
                    score += 2
                    break
    ...
    def display_context_help(self, context: HelpContext, verbose: bool = False):
        ...
        if primary_command in self.help_topics:
            ...
            # Title
            title = f"[bold cyan]{primary_command}[/bold cyan] - {topic.get('brief', '')}"
    ...
    def _display_general_help(self):
        ...
        for cmd, topic in sorted(self.help_topics.items()):
            shortcut = next((k for k, v in self.command_aliases.items() if v == cmd), "-")
    ...
    def _display_suggestions(self, context: HelpContext):
        ...
        if context.suggestions:
            ...
            self.console.print(f"[red]Unknown command:[/red] {' '.join(context.command_chain)}")
            self.console.print("[yellow]Use 'img --help' to see available commands[/yellow]")
    ...
    def _maybe_cleanup_cache(self, force: bool = False):
        ...
        if not force and current_time - self._last_cleanup < self.CACHE_CLEANUP_INTERVAL:
            return
    ...
    def _evict_lru_entries(self):
        ...
        entries_to_remove = len(self._help_cache) - self.MAX_CACHE_SIZE + 10  # Remove 10 extra
