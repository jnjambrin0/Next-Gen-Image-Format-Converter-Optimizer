...
class CLIProfiler:
    ...
    def display_profile_summary(self, profile_data: Dict[str, Any]):
        ...
        summary_table.add_row(
            "Operations Count",
            str(profile_data.get('operations_count', 0))
        )
        ...
        if perf:
            ...
            if cpu_data:
                summary_table.add_row(
                    "Average CPU Usage",
                    f"{cpu_data.get('average_percent', 0):.1f}%"
                )
                summary_table.add_row(
                    "Peak CPU Usage",
                    f"{cpu_data.get('peak_percent', 0):.1f}%"
                )
            
            if memory_data:
                summary_table.add_row(
                    "Memory Start",
                    f"{memory_data.get('start_mb', 0):.1f} MB"
                )
                summary_table.add_row(
                    "Memory Peak",
                    f"{memory_data.get('peak_mb', 0):.1f} MB"
                )
                summary_table.add_row(
                    "Memory Delta",
                    f"{memory_data.get('delta_mb', 0):+.1f} MB"
                )
            
            if io_data:
                summary_table.add_row(
                    "Data Read",
                    f"{io_data.get('read_mb', 0):.1f} MB"
                )
                summary_table.add_row(
                    "Data Written",
                    f"{io_data.get('write_mb', 0):.1f} MB"
                )
        ...
        if self.current_metrics:
            ...
            conversion_table.add_row("Output Size", f"{metrics_dict['output_mb']:.2f} MB")
            conversion_table.add_row("Compression Ratio", f"{metrics_dict['compression_ratio']:.2f}x")
            conversion_table.add_row("Processing Time", f"{metrics_dict['time_seconds']:.3f} seconds")
            conversion_table.add_row("Throughput", f"{metrics_dict['throughput_mbps']:.2f} MB/s")
