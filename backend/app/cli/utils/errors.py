...
class ErrorHandler:
    ...
    def __init__(self):
        self.known_commands = [
            "convert", "batch", "optimize", "analyze", "formats", "presets",
            "config", "aliases", "plugins", "history"
        ]
        
        self.known_formats = [
            "jpeg", "jpg", "png", "webp", "avif", "heif", "heic", "bmp", 
            "tiff", "gif", "jxl", "webp2"
        ]
    ...
    def suggest_format(self, incorrect: str) -> Optional[str]:
        ...
        matches = get_close_matches(incorrect.lower(), self.known_formats, n=1, cutoff=0.6)
        return matches[0] if matches else None
    ...
    def handle(self, error: Exception, console: Console):
        ...
        elif "invalid format" in error_msg.lower() or "unsupported format" in error_msg.lower():
            # Try to extract format and suggest alternatives
            words = error_msg.split()
            for word in words:
                suggestion = self.suggest_format(word)
                if suggestion:
                    suggestions.append(f"Did you mean format: [cyan]{suggestion}[/cyan]?")
        
        elif "connection" in error_msg.lower() or "api" in error_msg.lower():
            suggestions.append("Check if the API server is running: [cyan]cd backend && uvicorn app.main:app --port 8080[/cyan]")
...
def handle_api_error(error: Exception, console: Console):
    ...
    if hasattr(error, 'response'):
        ...
        if status == 404:
            console.print("[red]Error: API endpoint not found. Is the server running?[/red]")
            console.print("[dim]Start server: cd backend && uvicorn app.main:app --port 8080[/dim]")
        elif status == 401:
            console.print("[red]Error: Authentication failed. Check your API key.[/red]")
            ...
            console.print("[red]Error: Rate limit exceeded. Please wait before retrying.[/red]")
        elif status == 500:
        ...
        if "connection" in str(error).lower():
            console.print("[dim]Is the API server running? Check with: img config get api_url[/dim]")