...
def optimize_auto(
    ...
    preset: Annotated[
        ...
        typer.Option(
            "-p",
            "--preset",
            help="Optimization preset (web, print, archive, thumbnail)",
        ),
    ] = "balanced",
    output: Annotated[
    ...
    preview: Annotated[
        bool, typer.Option("--preview", help="Show before/after preview in terminal")
    ] = False,
    dry_run: Annotated[
        bool, typer.Option("--dry-run", help="Show optimization plan without executing")
    ] = False,
    profile: Annotated[
    ...
    if not validate_input_file(input_path):
        error_msg = format_with_emoji(
            f"Invalid or unsupported input file: {input_path}", "error"
        )
    ...
    settings_table.add_row(
        "Preset", f"{get_emoji('settings')} {preset}" if should_use_emoji() else preset
    )
    if quality_target:
    ...
    if dry_run:
        info_msg = format_with_emoji(
            "Dry run complete. No files were modified.", "info"
        )
    ...
    try:
        ...
        with InterruptableProgress(
            ...
            result = client.convert_image(
                image_data=image_data, request=request, input_filename=input_path.name
            )
            ...
            if progress.interrupted:
                error_msg = format_with_emoji(
                    "Optimization interrupted by user", "cancelled"
                )
...
def optimize_analyze(
    input_path: Annotated[
        Path, typer.Argument(help="Image file to analyze", exists=True)
    ],
):
    """
    ...
    try:
        ...
        if hasattr(analysis, "format"):
            analysis_table.add_row(
                "Format",
                (
                    f"{get_format_emoji(analysis.format)} {analysis.format.upper()}"
                    if should_use_emoji()
                    else analysis.format.upper()
                ),
                (
                    "Modern format"
                    if analysis.format in ["webp", "avif"]
                    else "Consider WebP/AVIF"
                ),
            )
        ...
        suggestions = [
            format_with_emoji(
                "Use 'img optimize auto' for automatic optimization", "tip"
            ),
