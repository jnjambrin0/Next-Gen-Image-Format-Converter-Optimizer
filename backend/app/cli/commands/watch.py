...
class WatchModeProcessor:
    ...
    def process_file(self, file_event: FileEvent):
        ...
        if self.output_dir:
            ...
            output_path = input_path.parent / (input_path.stem + f".{self.output_format}")
...
def start(
    directory: Annotated[
        Path,
        typer.Argument(help="Directory to watch")
    ],
    format: Annotated[
        str,
    ...
    exclude: Annotated[
        Optional[List[str]],
        typer.Option("--exclude", help="File patterns to exclude")
    ] = None,
    quality: Annotated[
        int,
        typer.Option("-q", "--quality", help="Output quality (1-100)")
    ] = 85,
    preset: Annotated[
        Optional[str],
        typer.Option("--preset", help="Optimization preset")
    ] = None,
    workers: Annotated[
        int,
        typer.Option("--workers", help="Max concurrent conversions")
    ] = 5,
    max_files: Annotated[
        int,
        typer.Option("--max-files", help="Max files in queue")
    ] = 100,
    debounce: Annotated[
        int,
        typer.Option("--debounce", help="Debounce delay in milliseconds")
    ] = 500,
    no_display: Annotated[
        bool,
        typer.Option("--no-display", help="Disable status display")
    ] = False
):
    """
    ...
    try:
        ...
        if no_display:
            ...
            with Live(display.update(watcher), refresh_per_second=1, console=console) as live:
                while watcher.status != WatcherStatus.STOPPED:
                    import time
                    time.sleep(0.5)
...
def test(
    directory: Annotated[
        Path,
        typer.Argument(help="Directory to test")
    ],
    filter: Annotated[
        Optional[List[str]],
        typer.Option("--filter", help="File patterns to include")
    ] = None
):
    """
