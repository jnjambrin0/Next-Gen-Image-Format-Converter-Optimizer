...
def help_command(
    ...
    query: Annotated[
        Optional[str],
        typer.Argument(help="Search query or command name")
    ] = None,
    verbose: Annotated[
        bool,
    ...
    examples: Annotated[
        bool,
        typer.Option("--examples", "-e", help="Show only examples")
    ] = False,
    errors: Annotated[
        bool,
        typer.Option("--errors", help="Show common errors and solutions")
    ] = False,
    search: Annotated[
        bool,
        typer.Option("--search", "-s", help="Search all help topics")
    ] = False,
    clear_cache: Annotated[
        bool,
        typer.Option("--clear-cache", help="Clear help cache", hidden=True)
    ] = False,
):
    """
...
def _show_command_help(command: str, verbose: bool, examples_only: bool, errors_only: bool):
    ...
    if errors_only:
        ...
        if errors:
            ...
            console.print(f"[yellow]No common errors documented for '{resolved}'[/yellow]")
        return
    ...
    # Description
    sections.append(f"[bold yellow]Description:[/bold yellow]\n{topic.get('description', '')}")
    ...
    if topic.get("common_errors"):
        ...
        for code, solution in list(topic["common_errors"].items())[:None if verbose else 2]:
