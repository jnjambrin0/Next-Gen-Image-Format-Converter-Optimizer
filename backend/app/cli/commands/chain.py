...
def chain(
    operations: Annotated[
        List[str],
        typer.Argument(help="Operations to chain (format:quality, resize:WxH, optimize, etc.)")
    ],
    ...
    if output_file:
        with open(output_file, 'wb') as f:
            f.write(image_data)
        console.print(f"[green]✓[/green] Output saved to: [cyan]{output_file}[/cyan]", file=sys.stderr)
    else:
        # Write to stdout
...
def pipe(
    ...
    quality: Annotated[
        Optional[int],
        typer.Option("-q", "--quality", help="Quality (1-100)")
    ] = None,
):
    """
    ...
    if sys.stdin.isatty():
        console.print("[red]Error: No input data. This command is for piping.[/red]", file=sys.stderr)
        console.print("[dim]Usage: cat image.jpg | img pipe -f webp > output.webp[/dim]", file=sys.stderr)
        raise typer.Exit(1)
