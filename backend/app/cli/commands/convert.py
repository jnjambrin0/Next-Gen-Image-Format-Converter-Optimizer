...
def convert_file(
    input_path: Annotated[
        Path, typer.Argument(help="Input image file path", exists=True)
    ],
    format: Annotated[
        str,
        typer.Option(
            "-f", "--format", help="Output format (webp, avif, jpeg, png, etc.)"
        ),
    ],
    output_path: Annotated[
        Optional[Path], typer.Option("-o", "--output", help="Output file path")
    ] = None,
    quality: Annotated[
        Optional[int],
        typer.Option(
            "-q", "--quality", min=1, max=100, help="Quality for lossy formats (1-100)"
        ),
    ] = None,
    preset: Annotated[
        Optional[str], typer.Option("-p", "--preset", help="Use optimization preset")
    ] = None,
    width: Annotated[
        Optional[int],
    ...
    height: Annotated[
        Optional[int],
        typer.Option(
            "-h", "--height", help="Resize to height (maintains aspect ratio)"
        ),
    ] = None,
    keep_metadata: Annotated[
        bool, typer.Option("--keep-metadata", help="Preserve image metadata")
    ] = False,
    optimize: Annotated[
        bool, typer.Option("--optimize", help="Apply automatic optimization")
    ] = False,
    dry_run: Annotated[
        bool, typer.Option("--dry-run", help="Preview operation without converting")
    ] = False,
    profile: Annotated[
    ...
    if not validate_input_file(input_path):
        console.print(
            f"[red]Error: Invalid or unsupported input file: {input_path}[/red]"
        )
        raise typer.Exit(1)
    ...
    if should_use_emoji():
        ...
        console.print(
            f"[bold green]{get_emoji('success')} To:[/bold green] {output_path.name} ({get_format_emoji(format)} {format.upper()})"
        )
    else:
        console.print(f"\n[bold cyan]Converting:[/bold cyan] {input_path.name}")
        console.print(
            f"[bold green]To:[/bold green] {output_path.name} ({format.upper()})"
        )
    ...
    if dry_run:
        ...
        table.add_row(
            "Format",
            f"{get_format_emoji(format) if should_use_emoji() else ''} {format.upper()}",
        )
        if quality:
    ...
    try:
        ...
        # Record in history
        record_command(
            f"convert {input_path} -f {format} -o {output_path}", success=True
        )

    except Exception as e:
        handle_api_error(e, console)
        record_command(
            f"convert {input_path} -f {format} -o {output_path}", success=False
        )
        raise typer.Exit(1)
...
def convert_stdin(
    ...
    try:
        ...
        if sys.stdin.isatty():
            console.print(
                "[red]Error: No input data. Pipe an image to this command.[/red]"
            )
            raise typer.Exit(1)
        ...
        if output:
            with open(output, "wb") as f:
                f.write(output_data)
            console.print(
                f"[green]✓[/green] Saved to: [cyan]{output}[/cyan]", file=sys.stderr
            )
        else:
            # Write to stdout
