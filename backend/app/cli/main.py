...
def main(
    ...
    debug: Annotated[
        bool,
        typer.Option("--debug", help="Enable debug mode with detailed errors")
    ] = False,
    output: Annotated[
        OutputFormat,
        typer.Option("--output", "-O", help="Output format for results")
    ] = OutputFormat.rich,
    language: Annotated[
        Optional[str],
...
# Add command groups
app.add_typer(convert.app, name="convert", help="Convert single images", no_args_is_help=True)
app.add_typer(batch.app, name="batch", help="Batch conversion operations", no_args_is_help=True)
app.add_typer(optimize.app, name="optimize", help="Intelligent image optimization", no_args_is_help=True)
app.add_typer(analyze.app, name="analyze", help="Analyze image properties", no_args_is_help=True)
app.add_typer(formats.app, name="formats", help="Manage supported formats", no_args_is_help=True)
app.add_typer(presets.app, name="presets", help="Manage conversion presets", no_args_is_help=True)
app.add_typer(chain.app, name="chain", help="Chain operations and piping", no_args_is_help=True)
...
def aliases(
    action: Annotated[
        Optional[str],
        typer.Argument(help="Action: list, add, remove")
    ] = "list",
    name: Annotated[
        Optional[str],
        typer.Argument(help="Alias name")
    ] = None,
    command: Annotated[
        Optional[str],
        typer.Argument(help="Command to alias")
    ] = None,
):
    """
    ...
    if action == "list":
        ...
        console.print(f"[green]✓[/green] Added alias [cyan]{name}[/cyan] → [green]{command}[/green]")
...
def plugins(
    action: Annotated[
        Optional[str],
        typer.Argument(help="Action: list, info, enable, disable")
    ] = "list",
    name: Annotated[
        Optional[str],
    ...
    if action == "list":
        ...
        if info:
            ...
            console.print(f"[yellow]Status:[/yellow] {'Enabled' if info['enabled'] else 'Disabled'}")
...
def history(
    action: Annotated[
        Optional[str],
        typer.Argument(help="Action: show, clear, undo, redo")
    ] = "show",
    count: Annotated[
        Optional[int],
        typer.Option("--count", "-n", help="Number of entries")
    ] = 10,
):
    """
