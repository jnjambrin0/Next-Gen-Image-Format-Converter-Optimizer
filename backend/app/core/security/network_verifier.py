...
class NetworkVerifier(NetworkIsolationChecker):
    ...
    async def verify_network_isolation(self) -> Dict[str, Any]:
        ...
        if self.strictness in [NetworkStrictness.STRICT, NetworkStrictness.PARANOID]:
            ...
            if connections_check["passed"]:
                ...
                self._network_status["isolated"] = False
                self._network_status["warnings"].extend(
                    connections_check.get("warnings", [])
                )
    ...
    def _verify_localhost_binding(self) -> VerificationResult:
        ...
        try:
            ...
            try:
                ...
                result["warnings"].append(
                    "Could not verify socket bindings (command failed)"
                )
    ...
    def _verify_no_active_connections(self) -> VerificationResult:
        ...
        try:
            ...
            cmd = (
                ["ss", "-tnp"] if os.path.exists("/usr/bin/ss") else ["netstat", "-tnp"]
            )
            ...
            try:
                ...
                for line in output.split("\n"):
                    if "ESTABLISHED" in line or "ESTAB" in line:
                        ...
                        if ("python" in line or str(os.getpid()) in line) and not (
                            "127.0.0.1" in line or "::1" in line
                        ):
                            result["passed"] = False
                            result["warnings"].append(
                                "Active non-localhost connection detected"
                            )
                            break

            except (subprocess.CalledProcessError, FileNotFoundError):
                result["warnings"].append(
                    "Could not verify active connections (command failed)"
                )
    ...
    def _verify_network_interfaces(self) -> VerificationResult:
        ...
        try:
            ...
            cmd = (
                ["ip", "addr"] if os.path.exists("/usr/bin/ip") else ["ifconfig", "-a"]
            )
            ...
            try:
                ...
                for line in lines:
                    ...
                    if current_interface and current_interface not in [
                        "lo",
                        "lo0",
                        "localhost",
                    ]:
                ...
                result["warnings"].append(
                    "Could not verify network interfaces (command failed)"
                )
