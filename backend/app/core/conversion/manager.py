...
class ConversionManager:
    ...
    def _initialize_handlers(self) -> None:
        ...
        self.register_handler("heif", HeifHandler)  # Also registers heic, heix, hevc, hevx via aliases
        ...
        try:
            self.register_handler("jxl", JxlHandler)  # Also registers jpegxl, jpeg_xl via aliases
    ...
    async def convert_image(
        ...
        output_format_str = request.output_format.value if hasattr(request.output_format, 'value') else str(request.output_format)
        ...
        try:
            ...
            output_format_str = request.output_format.value if hasattr(request.output_format, 'value') else str(request.output_format)
            actual_output_format, is_fallback = self.get_format_with_fallback(output_format_str)
            ...
            if self.security_engine and settings.enable_sandboxing:
                ...
                output_data = await self._process_sandboxed(
                    processed_input_data, input_format, request, result, actual_output_format
                )
            else:
            ...
            await stats_collector.record_conversion(
                input_format=input_format,
                output_format=actual_output_format if 'actual_output_format' in locals() else output_format_str,
                input_size=len(input_data),
                processing_time=result.processing_time,
                success=False,
                error_type=type(e).__name__
