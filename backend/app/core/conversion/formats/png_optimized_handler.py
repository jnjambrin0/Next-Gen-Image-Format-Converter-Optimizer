...
class PNGOptimizedHandler(PNGHandler):
    ...
    def save_image(
        ...
        try:
            optimized_data = self._optimize_png(
            ...
            logger.warning(
                "PNG optimization failed, falling back to regular PNG",
                error=str(e)
            )
            output_buffer.write(temp_buffer.getvalue())
            output_buffer.seek(0)
    ...
    def _optimize_png(self, png_data: bytes, settings: ConversionSettings) -> bytes:
        ...
        if self.pngquant.is_available and self.optipng.is_available and settings.quality < 100:
    ...
    def _optimize_with_pngquant(self, png_data: bytes, settings: ConversionSettings) -> bytes:
        ...
        try:
            ...
            args = [
                ...
                "--speed", "1" if settings.optimize else "3",
            ...
            result = self.pngquant.execute(
                args,
                timeout=settings.conversion_timeout if hasattr(settings, 'conversion_timeout') else 30
            )
    ...
    def _optimize_with_optipng(self, png_data: bytes, settings: ConversionSettings) -> bytes:
    ...
    def _optimize_combined(self, png_data: bytes, settings: ConversionSettings) -> bytes:
