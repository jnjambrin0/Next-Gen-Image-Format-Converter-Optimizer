        ...
        async with self._analysis_semaphore:
            try:
                ...
                if width > MAX_PROCESS_SIZE or height > MAX_PROCESS_SIZE:
                    ...
                    logger.debug(
                        f"Downsampled images from {width}x{height} to {new_width}x{new_height}"
                    )
                ...
                # Calculate SSIM
                if calculate_ssim:
                    ssim_score = await self._calculate_ssim(
                        original_array, optimized_array
                    )
                    result["ssim_score"] = float(ssim_score)

                # Calculate PSNR
                if calculate_psnr:
                    psnr_value = await self._calculate_psnr(
                        original_array, optimized_array
                    )
