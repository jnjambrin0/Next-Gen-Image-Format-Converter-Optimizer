    ...
    async def optimize_regions(
        ...
        **kwargs
        ...
        if image.width < MIN_REGION_SIZE * 2 or image.height < MIN_REGION_SIZE * 2:
            ...
            if conversion_func:
                return await conversion_func(image_data, output_format, quality=base_quality, **kwargs)
            return image_data
    ...
    async def _detect_regions(
        ...
        async with self._region_semaphore:
            try:
                ...
                classification = await self.intelligence_engine.classify_content(image_data)
                ...
                if detect_text and classification.text_regions:
                    for text in classification.text_regions:
                        regions.append(Region(
                            ...
                            bbox=(text.x, text.y, text.x + text.width, text.y + text.height),
                ...
                if detect_foreground:
                    ...
                    regions.append(Region(
                        ...
                        bbox=(fg_margin_x, fg_margin_y, width - fg_margin_x, height - fg_margin_y),
                        confidence=0.8,
                    ...
                    regions.append(Region(
                        type=RegionType.BACKGROUND,
                        bbox=(width - fg_margin_x, fg_margin_y, width, height - fg_margin_y),
                        confidence=0.7,
                        quality_factor=self.quality_factors[RegionType.BACKGROUND]
                    ))
