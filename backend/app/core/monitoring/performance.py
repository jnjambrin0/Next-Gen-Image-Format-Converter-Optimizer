...
class ConversionMetrics:
    ...
    def to_json(self) -> dict:
        ...
        return {
            ...
            'compression_ratio': round(self.file_size / self.output_size, 2) if self.output_size > 0 else 0,
            'throughput_mbps': round((self.file_size / 1024 / 1024) / self.processing_time, 2) if self.processing_time > 0 else 0,
...
class BatchMetrics:
    ...
    def worker_efficiency(self) -> float:
        ...
        if self.worker_count > 0 and self.elapsed_time > 0:
            ideal_time = self.average_time_per_file * self.total_files / self.worker_count
    ...
    def to_json(self) -> dict:
        ...
        return {
            ...
            'summary': {
                ...
                'success_rate': round((self.completed_files / self.total_files * 100) if self.total_files > 0 else 0, 2),
            ...
            'performance': {
                ...
                'compression_ratio': round(self.total_input_size / self.total_output_size, 2) if self.total_output_size > 0 else 0,
                'peak_memory_mb': round(self.peak_memory_mb, 2)
            },
            'file_details': [m.to_json() for m in self.file_metrics[-10:]],  # Last 10 files
...
class PerformanceMonitor:
    ...
    def stop(self) -> Dict[str, Any]:
        ...
        stats = {
            'cpu': {
                'average_percent': round(sum(cpu_samples) / len(cpu_samples), 2) if cpu_samples else 0,
            ...
            'memory': {
                ...
                'average_mb': round(sum(memory_samples) / len(memory_samples), 2) if memory_samples else 0,
                'delta_mb': round((current_memory - self._start_memory) / 1024 / 1024, 2)
            },
            'io': {
