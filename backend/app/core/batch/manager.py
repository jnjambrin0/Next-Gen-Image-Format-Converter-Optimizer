...
class BatchManager:
    ...
    def __init__(self):
        ...
        self._max_concurrent_per_job = min(
            self._num_workers, settings.max_concurrent_conversions
        )
    ...
    def _update_memory_metrics(self, job_id: str):
        ...
        if worker_id not in metrics["worker_activity"]:
            metrics["worker_activity"][worker_id] = {
                "files_processed": 0,
                "total_time": 0,
            }
    ...
    async def create_job(
        ...
        for i, (item, file_data) in enumerate(zip(batch_job.items, file_data_list)):
            ...
            conv_settings = None
            if any(
                k in batch_job.settings
                for k in ["quality", "preserve_metadata", "optimization_mode"]
            ):
                conv_settings = ConversionSettings(
                    quality=batch_job.settings.get("quality"),
                    preserve_metadata=batch_job.settings.get(
    ...
    async def _process_task(self, task: BatchWorkerTask, job: BatchJob) -> None:
        ...
        try:
            ...
            if task.job_id in self._job_metrics:
                self._job_metrics[task.job_id]["file_times"].append(
                ...
                if worker_id in self._job_metrics[task.job_id]["worker_activity"]:
                    self._job_metrics[task.job_id]["worker_activity"][worker_id][
                        "total_time"
                    ] += file_processing_time
    ...
    async def _send_progress(
        self, job_id: str, file_index: int, item: BatchItem
    ) -> None:
    ...
    async def _simulate_progress(
        self, job_id: str, file_index: int, item: BatchItem, estimated_time: float = 3.0
    ) -> None:
    ...
    def _check_job_completion(self, job: BatchJob) -> None:
        ...
        if job.status in [BatchStatus.COMPLETED, BatchStatus.FAILED]:
            ...
            asyncio.create_task(
                batch_history_service.update_job_status(
                    job_id=job.job_id,
                ...
                worker_stats.append(
                    {
                        ...
                        "efficiency": (
                            (avg_time_per_file / avg_time * 100)
                            if avg_time > 0 and avg_time_per_file > 0
                            else 0
                        ),
                    }
                )
        ...
        return {
            ...
            "throughput": {
                ...
                "estimated_completion_seconds": (
                    (job.total_files / throughput - elapsed_time)
                    if throughput > 0 and job
                    else 0
                ),
            },
    ...
    async def start_processing(self, job_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Started processing for job {job_id} with {len(file_data_list)} files"
            )
            return True
    ...
    async def cleanup_pending_job(self, job_id: str) -> None:
        ...
        if job and job.status == BatchStatus.PENDING:
            ...
            self.logger.info(
                f"Marked job {job_id} as failed due to processing startup failure"
            )
