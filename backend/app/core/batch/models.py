...
class BatchItem(BaseModel):
    ...
    output_size: Optional[int] = Field(None, description="Size of converted file in bytes")
...
class BatchJob(BaseModel):
    ...
    settings: Dict[str, Any] = Field(default_factory=dict, description="Conversion settings")
    items: List[BatchItem] = Field(default_factory=list, description="Individual file items")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    completed_at: Optional[datetime] = None
    ...
    def processing_files(self) -> int:
        ...
        return sum(1 for item in self.items if item.status == BatchItemStatus.PROCESSING)
    ...
    def update_status(self) -> None:
        ...
        if all(item.status in [BatchItemStatus.COMPLETED, BatchItemStatus.FAILED] 
...
class BatchStatusResponse(BaseModel):
    ...
    download_url: Optional[str] = Field(None, description="URL to download results (if completed)")
...
class BatchJobStatus(BaseModel):
    ...
    created_at: Optional[str] = Field(None, description="Job creation time (ISO format)")
...
class BatchResult(BaseModel):
    ...
    job_id: str = Field(..., description="Batch job ID")
    total_files: int = Field(..., description="Total number of files")
    successful_files: List[Dict[str, Any]] = Field(..., description="Successfully converted files")
    failed_files: List[Dict[str, Any]] = Field(..., description="Failed files with errors")
