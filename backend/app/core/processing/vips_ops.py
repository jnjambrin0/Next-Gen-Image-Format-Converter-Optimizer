...
class VipsOperations:
    ...
    def _check_memory_limit(self) -> None:
        ...
        if memory_delta > MAX_MEMORY_MB:
            raise ConversionError(f"Memory limit exceeded: {memory_delta:.1f}MB > {MAX_MEMORY_MB}MB")
    ...
    def process_large_image(
        ...
        **kwargs
        ...
        if not self.vips_available:
            return self._process_with_pil(input_path, output_format, quality, resize, **kwargs)
        ...
        try:
            ...
            output_options = self._get_vips_save_options(output_format, quality, **kwargs)
    ...
    def monitor_memory(self, operation_name: str) -> Dict[str, float]:
        ...
        stats = {
            ...
            "limit_mb": MAX_MEMORY_MB,
            "usage_percent": (memory_delta / MAX_MEMORY_MB) * 100 if MAX_MEMORY_MB > 0 else 0
        }
