...
class TextDetector:
    ...
    def _morphological_close(
        self, binary: np.ndarray, kernel: np.ndarray
    ) -> np.ndarray:
    ...
    def _find_text_components(
        self, binary: np.ndarray, scale_back: float
    ) -> List[BoundingBox]:
    ...
    def _bfs_component(
        self, binary: np.ndarray, visited: np.ndarray, start_y: int, start_x: int
    ) -> List[Tuple[int, int]]:
        """BFS to find connected component."""
        h, w = binary.shape
    ...
    def _extract_text_regions(
        ...
        def flood_fill(start_y: int, start_x: int) -> List[Tuple[int, int]]:
            ...
            while stack:
                ...
                if (
                    y < 0
                    or y >= binary_map.shape[0]
                    or x < 0
                    or x >= binary_map.shape[1]
                ):
                    continue
            ...
            for x in range(binary_map.shape[1]):
                if binary_map[y, x] == 1 and not visited[y, x]:
                    ...
                    if len(points) > self.min_area:
                        ...
                        regions.append(
                            (min_x, min_y, max_x - min_x, max_y - min_y, confidence)
                        )
    ...
    def _merge_overlapping_regions(
        ...
        for region in sorted_regions[1:]:
            ...
            vertical_overlap = (
                current.y <= region.y <= current.y + current.height
                or region.y <= current.y <= region.y + region.height
            )

            horizontal_overlap = (
                current.x <= region.x <= current.x + current.width
                or region.x <= current.x <= region.x + region.width
            )
            ...
            if (vertical_overlap and horizontal_overlap) or (
                same_line and close_horizontally
            ):
                # Merge regions
