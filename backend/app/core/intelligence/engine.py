...
class IntelligenceEngine:
    ...
    def __init__(
        ...
        if models_dir:
            try:
                ...
                if ".." in str(models_path):
                    logger.warning("Invalid models directory path - contains parent references")
        ...
        # Concurrency control
        self._classification_semaphore = asyncio.Semaphore(MAX_CONCURRENT_CLASSIFICATIONS)
    ...
    def _load_models(self) -> None:
        ...
        try:
            ...
            content_model_path = self.models_dir / "content" / "mobilenet_v3_content.onnx"
            ...
            if face_model_path.exists() and face_model_path.stat().st_size <= INTELLIGENCE_MODEL_MAX_SIZE:
            ...
            if text_model_path.exists() and text_model_path.stat().st_size <= INTELLIGENCE_MODEL_MAX_SIZE:
    ...
    async def classify_content(
        ...
        if len(image_data) > 100 * 1024 * 1024:  # 100MB max
            raise create_file_error("input_too_large", size_mb=len(image_data) / 1024 / 1024)
            ...
            async with self._cache_lock:
                if image_hash in self._cache:
                    ...
                    try:
                        ...
                        result_copy.processing_time_ms = (time.time() - start_time) * 1000
        ...
        async with self._classification_semaphore:
            ...
            async with self._memory_lock:
                ...
                if self._total_memory_used + estimated_memory > MAX_MEMORY_PER_CLASSIFICATION * MAX_CONCURRENT_CLASSIFICATIONS:
                    self._active_classifications -= 1
                    raise create_verification_error("memory_limit_exceeded")
                self._total_memory_used += estimated_memory
            
            try:
                return await self._classify_with_protection(image_data, image_hash, start_time, debug)
            finally:
                # Release memory tracking
                async with self._memory_lock:
                    self._active_classifications -= 1
                    self._total_memory_used = max(0, self._total_memory_used - estimated_memory)
        ...
        try:
            ...
            try:
                ...
                if decompression_ratio > 1000:  # 1000:1 max ratio for extreme cases
                    logger.warning(f"Suspicious compression ratio detected: {decompression_ratio:.1f}:1")
                ...
                if image.width > MAX_IMAGE_DIMENSION or image.height > MAX_IMAGE_DIMENSION:
                    ...
                    logger.debug(f"Downsampled large image for classification from {(image.width, image.height)} to {new_size}")
            ...
            if quick_result.confidence >= self.cascade_threshold:
                logger.debug(f"Quick classifier confident: {quick_result.content_type} ({quick_result.confidence:.2f})")
            ...
            logger.warning(f"Classification timed out after {INTELLIGENCE_TIMEOUT_MS}ms")
    ...
    async def _run_quick_classification(self, image: Image.Image) -> ClassificationResult:
        """Run quick rule-based classification."""
        return await asyncio.get_event_loop().run_in_executor(
            None,
            self.quick_classifier.classify,
            image
        )
    ...
    def _combine_results(
        self,
        quick_result: ClassificationResult,
        ...
        if quick_result.content_type == ml_result.content_type:
            combined_confidence = min(0.99, (quick_result.confidence + ml_result.confidence) / 2 + 0.1)
            return ClassificationResult(
                content_type=quick_result.content_type,
                confidence=combined_confidence,
                processing_time_ms=quick_result.processing_time_ms + ml_result.processing_time_ms,
    ...
    async def _create_classification_result(
        ...
        if text_regions and hasattr(self.text_detector, 'calculate_text_density'):
            text_density = self.text_detector.calculate_text_density(image, text_regions)
    ...
    async def _ml_classification(self, image: Image.Image) -> ContentClassification:
        ...
        outputs = self.content_classifier.run(
            None,
            {self.content_classifier.get_inputs()[0].name: processed_image}
        )
        ...
        content_types = [ContentType.PHOTO, ContentType.ILLUSTRATION, 
                        ContentType.SCREENSHOT, ContentType.DOCUMENT]
    ...
    async def _heuristic_classification(
        ...
        is_screenshot = (
            (width in [1920, 1366, 1440, 2560, 3840] and 
             height in [1080, 768, 900, 1440, 2160]) or
            (aspect_ratio > 1.5 and aspect_ratio < 2.0)
        )
        ...
        if image.mode in ['L', '1']:  # Grayscale or binary
            ...
            if len(hist) >= 768:  # RGB histogram
                ...
                extreme_pixels = (r_hist[0] + r_hist[255] + 
                                g_hist[0] + g_hist[255] + 
                                b_hist[0] + b_hist[255]) / 3
    ...
    def _calculate_complexity(self, image: Image.Image) -> float:
        ...
        try:
            ...
            edge_density = (edges_x + edges_y) / total_possible_edges if total_possible_edges > 0 else 0
    ...
    def recommend_settings(
        ...
        content_settings = {
            ContentType.PHOTO: {
                ...
                "specific": {
                    ...
                    "jpegxl": {
                        "distance": 1.0,  # Lower = better quality
                        "effort": 7
                    }
                }
            },
        ...
        content_config = content_settings.get(
            content_type,
            content_settings[ContentType.PHOTO]  # Default
        )
    ...
    async def _add_to_cache(self, key: str, value: ContentClassification) -> None:
        ...
        async with self._cache_lock:
            ...
            if len(self._cache) >= MAX_CACHE_SIZE:
                ...
                logger.debug(f"Evicted oldest cache entry, cache size: {len(self._cache)}")
            
            # Add new item at end
            self._cache[key] = value
    
    def _secure_clear_classification(self, classification: ContentClassification) -> None:
