...
class FaceDetector:
    ...
    def __init__(self, model_session: Optional[Any] = None, input_size: int = 128) -> None:
    ...
    def _preprocess_for_model(self, image: Image.Image) -> np.ndarray:
        ...
        resized = image.resize((self.input_size, self.input_size), Image.Resampling.LANCZOS)
    ...
    def _generate_anchors(self) -> np.ndarray:
        ...
        layer_id = 0
        for stride, anchor_count in zip(self.config['strides'], self.config['anchor_counts']):
    ...
    def _decode_detections(self, raw_outputs: np.ndarray) -> List[Tuple[float, float, float, float, float]]:
