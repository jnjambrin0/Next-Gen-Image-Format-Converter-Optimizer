...
class ImagePreprocessor:
    ...
    def detect_edges_sobel_optimized(
        ...
        if CV2_AVAILABLE:
            ...
            edges_x = signal.convolve2d(
                gray_array, 
                ImagePreprocessor.SOBEL_X, 
                mode='valid',
                boundary='symm'
            )
            edges_y = signal.convolve2d(
                gray_array,
                ImagePreprocessor.SOBEL_Y,
                mode='valid',
                boundary='symm'
            )
        else:
            # Manual convolution fallback
            edges_x = ImagePreprocessor._manual_convolve2d(gray_array, ImagePreprocessor.SOBEL_X)
            edges_y = ImagePreprocessor._manual_convolve2d(gray_array, ImagePreprocessor.SOBEL_Y)
        ...
        return {
            ...
            'edge_straightness': float((horizontal_strength + vertical_strength) / 
    ...
    def analyze_color_distribution(
        ...
        gray_diff = np.abs(sampled_pixels[:, 0] - sampled_pixels[:, 1]) + \
                   np.abs(sampled_pixels[:, 1] - sampled_pixels[:, 2])
    ...
    def detect_ui_patterns(gray_array: np.ndarray) -> Dict[str, float]:
        ...
        for y in range(0, h - block_size, block_size // 2):
            for x in range(0, w - block_size, block_size // 2):
                ...
                if (top_edge + bottom_edge + left_edge + right_edge) > 40 and interior_variance < 100:
                    rect_score += 1
        
        total_blocks = ((h - block_size) // (block_size // 2)) * ((w - block_size) // (block_size // 2))
        ...
        return {
            ...
            'ui_score': float((horizontal_lines / h + vertical_lines / w + rect_ratio) / 3)
        }
