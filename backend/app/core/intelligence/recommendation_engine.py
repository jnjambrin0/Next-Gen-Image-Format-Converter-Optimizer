...
class RecommendationEngine:
    ...
    async def generate_recommendations(
        self, request: RecommendationRequest, max_recommendations: int = 3
        ...
        if not isinstance(request, RecommendationRequest):
            raise create_file_error(
                "invalid_input_type", "Invalid recommendation request type"
            )
        ...
        async with self._semaphore:
            ...
            for format_enum in available_formats:
                ...
                if cached_score is not None:
                    ...
                    scoring_tasks.append(
                        (
                            format_enum,
                            self._calculate_format_score(format_enum, request),
                        )
                    )
            ...
            sorted_formats = sorted(
                format_scores.items(), key=lambda x: x[1], reverse=True
            )[:max_recommendations]
            ...
            logger.info(
                f"Generated format recommendations in {processing_time_ms:.1f}ms"
            )
    ...
    async def _calculate_format_score(
        self, format_enum: OutputFormat, request: RecommendationRequest
    ) -> float:
        ...
        content_score = self._get_content_score(
            format_enum, request.content_classification.primary_type
        )

        # Use case score
        use_case_score = self._get_use_case_score(
            format_enum, request.use_case, request.prioritize
        )

        # Source compatibility score
        compatibility_score = self._get_compatibility_score(
            format_enum, request.original_format
        )

        # User preference score
        preference_score = await self._get_preference_score(
            format_enum, request.content_classification.primary_type, request.use_case
        )
    ...
    def _get_content_score(
        self, format_enum: OutputFormat, content_type: ContentType
    ) -> float:
    ...
    def _get_use_case_score(
        ...
        if prioritize:
            if prioritize == "size":
                ...
                weights["browser_support"] = min(
                    1.0, weights.get("browser_support", 0.5) * 1.5
                )
    ...
    def _get_compatibility_score(
        self, output_format: OutputFormat, input_format: InputFormat
    ) -> float:
        ...
        # JPEG variants
        if input_format in [InputFormat.JPEG, InputFormat.JPG] and output_format in [
            OutputFormat.JPEG,
            OutputFormat.WEBP,
            OutputFormat.AVIF,
        ]:
        ...
        if input_format == InputFormat.PNG and output_format in [
            OutputFormat.PNG,
            OutputFormat.WEBP,
            OutputFormat.AVIF,
        ]:
    ...
    async def _get_preference_score(
        ...
        try:
            preference = await self.preference_callback(
                content_type, format_enum, use_case
            )
    ...
    async def _create_recommendation(
        self, format_enum: OutputFormat, score: float, request: RecommendationRequest
    ) -> FormatRecommendation:
        """Create detailed format recommendation."""
        characteristics = self.FORMAT_CHARACTERISTICS.get(format_enum)

        # Generate trade-off analysis
        trade_offs = self._analyze_trade_offs(format_enum, request)

        # Generate reasons
        reasons = self._generate_reasons(format_enum, request, characteristics)

        # Generate pros and cons
        pros, cons = self._generate_pros_cons(format_enum, request, characteristics)
        ...
        return FormatRecommendation(
            ...
            quality_score=(
                characteristics.quality_preservation if characteristics else 0.7
            ),
            compatibility_score=(
                characteristics.browser_support if characteristics else 0.5
            ),
    ...
    def _analyze_trade_offs(
        ...
        elif content_type == ContentType.DOCUMENT and format_enum in [
            OutputFormat.JPEG
        ]:
        ...
        if (
            request.content_classification.has_text
            and not characteristics.features.get("lossless")
        ):
            feature_score *= 0.8
        if (
            characteristics.features.get("transparency")
            and content_type == ContentType.ILLUSTRATION
        ):
            feature_score *= 1.2
    ...
    def _generate_reasons(
        ...
        if content_score >= 0.9:
            if content_type == ContentType.PHOTO:
                reasons.append(
                    f"Excellent for photographs with {int(content_score * 100)}% suitability"
                )
        ...
        if request.use_case:
            if (
                request.use_case == UseCaseType.WEB
                and characteristics
                and characteristics.browser_support >= 0.8
            ):
                reasons.append(
                    f"Wide browser support ({int(characteristics.browser_support * 100)}%)"
                )
            elif (
                request.use_case == UseCaseType.PRINT
                and characteristics
                and characteristics.quality_preservation >= 0.9
            ):
                reasons.append("High quality preservation for print reproduction")
            elif (
                request.use_case == UseCaseType.ARCHIVE
                and characteristics
                and characteristics.future_proof >= 0.8
            ):
        ...
        if characteristics and characteristics.compression_efficiency >= 0.9:
            reasons.append(
                f"Superior compression (~{int((1 - characteristics.compression_efficiency) * 100 + 30)}% smaller files)"
            )
    ...
    def _generate_pros_cons(
        ...
        if (
            not characteristics.features.get("transparency")
            and request.content_classification.primary_type == ContentType.ILLUSTRATION
        ):
            cons.append("No transparency support")
        if (
            format_enum in [OutputFormat.JPEG]
            and request.content_classification.has_text
        ):
