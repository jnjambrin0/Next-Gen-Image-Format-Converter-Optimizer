...
from app.core.security.errors_simplified import create_file_error, create_rate_limit_error
...
class UserPreferenceTracker:
            ...
            try:
                with self._get_db() as conn:
                    ...
                    count_result = conn.execute("""
                        ...
                        WHERE content_type = ?
                    """, (content_type.value,)).fetchone()
                    
                    if count_result and count_result['count'] >= self.MAX_PREFERENCES_PER_TYPE:
                ...
                raise create_file_error(operation="database", reason="Failed to record preference")
    ...
    def _update_preference_summary(
        ...
        result = conn.execute("""
            ...
            WHERE content_type = ? AND 
                  ...
                  format = ?
        """, (content_type, use_case, use_case, format_chosen)).fetchone()
    ...
    async def get_preference_score(
        ...
        """
        # Check cache
        cache_key = (content_type.value, format_option.value, use_case.value if use_case else None)
        current_time = time.time()
        
        if (cache_key in self._preference_cache and 
        ...
        with self._lock:
            try:
                with self._get_db() as conn:
                    ...
                    if not result or result['total_selections'] < self.PREFERENCE_THRESHOLD:
                        score = 0.0
                    else:
                        # Apply time decay
                        days_old = (current_time - result['last_selected']) / 86400
                        decay_factor = max(0.0, 1.0 - (days_old / self.PREFERENCE_DECAY_DAYS))
        ...
        with self._lock:
            try:
                with self._get_db() as conn:
                    ...
                    query += " ORDER BY preference_score DESC, last_selected DESC LIMIT ?"
                    params.append(limit)
                    ...
                    for row in results:
                        try:
                            ...
                            preferences.append(UserFormatPreference(
    ...
    def _check_rate_limit(self) -> bool:
        ...
        self._rate_limit_tracker = {
            t: count for t, count in self._rate_limit_tracker.items()
            if t > window_start
        }
