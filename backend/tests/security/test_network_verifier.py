...
class TestNetworkVerifier:
    ...
    async def test_standard_verification(self):
        ...
        with patch("subprocess.check_output") as mock_output:
            mock_output.return_value = "tcp    LISTEN     0      128    127.0.0.1:8000"
    ...
    async def test_strict_verification(self):
        ...
        with patch("subprocess.check_output") as mock_output:
            ...
            mock_output.side_effect = [
                "tcp    LISTEN     0      128    127.0.0.1:8000",  # localhost binding
                "tcp    ESTAB      0      0      127.0.0.1:8000",  # active connections
            ]
    ...
    def test_localhost_binding_check_failure(self) -> None:
        ...
        with patch("subprocess.check_output") as mock_output:
            ...
            mock_output.return_value = (
                "tcp    LISTEN     0      128    0.0.0.0:8000     python"
            )
    ...
    def test_active_connections_check_failure(self) -> None:
        ...
        with patch("subprocess.check_output") as mock_output:
            ...
            mock_output.return_value = (
                "tcp    ESTAB    0    0    192.168.1.1:443    python"
            )
...
"""
    ...
    async def test_security_event_recording(self):
        ...
        verifier = NetworkVerifier(
            strictness=NetworkStrictness.PARANOID, security_tracker=mock_tracker
        )
