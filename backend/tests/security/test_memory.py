...
class TestSecureMemoryManager:
    ...
    @pytest.mark.skipif(os.name != 'posix', reason="Memory locking only on POSIX systems")
...
class TestMemorySecurityIntegration:
    ...
    @pytest.mark.skipif(not hasattr(resource, 'RLIMIT_AS'), reason="Memory limits not supported")
