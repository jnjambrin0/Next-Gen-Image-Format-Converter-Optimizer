...
class TestNetworkConnection:
    ...
    def test_localhost_detection(self):
        ...
        conn1 = NetworkConnection("tcp", "127.0.0.1", 8080, "127.0.0.1", 45678, "ESTABLISHED")
        assert conn1.is_localhost() is True
        
        conn2 = NetworkConnection("tcp", "::1", 8080, "::1", 45678, "ESTABLISHED")
        assert conn2.is_localhost() is True
        
        # External connection
        conn3 = NetworkConnection("tcp", "192.168.1.100", 8080, "8.8.8.8", 443, "ESTABLISHED")
        assert conn3.is_localhost() is False
    
    def test_to_dict_privacy(self):
        """Test that to_dict doesn't leak sensitive info."""
        conn = NetworkConnection(
            "tcp", "192.168.1.100", 8080, "8.8.8.8", 443, 
            "ESTABLISHED", pid=1234, process_name="python"
        )
        ...
        assert 8080 not in data.values()
...
class TestNetworkMonitor:
    ...
    async def test_baseline_establishment(self):
        ...
        mock_connections = [
            NetworkConnection("tcp", "127.0.0.1", 8080, "0.0.0.0", 0, "LISTEN"),
            NetworkConnection("tcp", "127.0.0.1", 45678, "127.0.0.1", 8080, "ESTABLISHED"),
            NetworkConnection("tcp", "192.168.1.100", 45679, "8.8.8.8", 443, "ESTABLISHED")
        ]
        
        with patch.object(monitor, '_get_current_connections', return_value=mock_connections):
    ...
    def test_parse_connection_line(self):
        ...
        line1 = "tcp    ESTAB      0      0      127.0.0.1:8080      127.0.0.1:45678     users:((\"python\",pid=1234,fd=3))"
        conn1 = monitor._parse_connection_line(line1)
        
        assert conn1 is not None
        assert conn1.protocol == "tcp"
        assert conn1.local_addr == "127.0.0.1"
        assert conn1.local_port == 8080
        ...
        line2 = "tcp        0      0 0.0.0.0:8080            0.0.0.0:*               LISTEN      1234/python"
    ...
    def test_parse_address(self):
        ...
        # IPv4
        addr, port = monitor._parse_address("127.0.0.1:8080")
        assert addr == "127.0.0.1"
        assert port == 8080
        
        # IPv6
        addr, port = monitor._parse_address("[::1]:8080")
        assert addr == "::1"
        assert port == 8080
    ...
    async def test_violation_detection(self):
        ...
        with patch.object(monitor, '_get_current_connections', return_value=baseline_conns):
            await monitor._establish_baseline()
        
        # Simulate violation (external connection)
        violation_conns = baseline_conns + [
            NetworkConnection(
                "tcp", "192.168.1.100", 45679, "8.8.8.8", 443, 
                "ESTABLISHED", pid=os.getpid()
            )
        ]
        
        with patch.object(monitor, '_get_current_connections', return_value=violation_conns):
    ...
    async def test_monitor_loop_error_handling(self):
        ...
        with patch.object(monitor, '_check_connections', side_effect=Exception("Test error")):
            monitor._monitoring = True
