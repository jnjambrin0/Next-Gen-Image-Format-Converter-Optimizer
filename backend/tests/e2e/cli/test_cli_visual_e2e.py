...
class TestCLIVisualFeatures:
    ...
    def test_convert_with_themed_output(self, cli_runner, sample_images, ansi_parser, progress_validator):
        ...
        if "TERM_PROGRAM" in os.environ:
            assert ansi_parser.has_emoji(result.stdout), "Should show emoji in supported terminals"
        
        # Verify progress indicators
        assert "Converting" in result.stdout
        assert progress_validator.has_progress_bar(result.stdout) or "✓" in result.stdout
    ...
    def test_ascii_preview_generation(self, cli_runner, sample_images, ansi_parser):
        ...
        if result.returncode != 0:
            ...
            preview = create_ascii_preview(input_file, width=40, height=20, mode="ascii")
            assert preview, "Should generate ASCII preview"
            assert len(preview) > 0
            
            # Check for ASCII characters
            ascii_chars = set(" .:-=+*#%@")
            assert any(c in preview for c in ascii_chars), "Should contain ASCII art characters"
    ...
    def test_batch_conversion_with_progress(self, cli_runner, sample_images, progress_validator):
    ...
    def test_smart_table_formatting(self, cli_runner, sample_images, ansi_parser):
        ...
        if result.returncode == 0:
            ...
            if table_lines:
                ...
                assert any('─' in line or '-' in line or '═' in line for line in table_lines)
                
                # Should have headers
                assert any("Format" in line or "INPUT" in line for line in table_lines)
    
    def test_emoji_support_and_fallback(self, cli_runner, sample_images, ansi_parser, terminal_configs):
        ...
        result_with_emoji = cli_runner.run_img_command(
            f"convert file {input_file} -f webp",
            env=emoji_env
        )
        ...
        if "NO_COLOR" in no_emoji_env and no_emoji_env["NO_COLOR"] == "1":
            assert not ansi_parser.has_ansi_codes(result_no_emoji.stdout), "Should not have colors when NO_COLOR=1"
    ...
    def test_theme_switching(self, cli_runner, ansi_parser, theme_validator):
        ...
        if result.returncode == 0:
            ...
            if result_set.returncode == 0:
                assert "dark" in result_set.stdout.lower() or "set" in result_set.stdout.lower()
        
        # Verify themed output
        assert theme_validator.has_styled_output(result.stdout) or True, "Should have some styling"
    ...
    def test_tui_launch(self, cli_runner):
        ...
        assert "Interactive Mode" in result.stdout or "Textual" in result.stdout or result.returncode != 0
    ...
    def test_error_display_with_styling(self, cli_runner, ansi_parser):
        ...
        if ansi_parser.has_ansi_codes(result.stdout) or ansi_parser.has_ansi_codes(result.stderr):
            colors = ansi_parser.extract_colors(result.stdout + result.stderr)
            # Check for red color code (31 or 91)
            assert any('31' in c or '91' in c for c in colors) or True, "Errors might be in red"
    
    def test_terminal_capability_detection(self, cli_runner, terminal_configs, ansi_parser):
        ...
        for config in terminal_configs:
            ...
            if config["name"] == "full_featured":
                ...
                assert ansi_parser.has_ansi_codes(result.stdout) or result.returncode != 0
            elif config["name"] == "no_color":
                # Should have minimal colors
                assert not ansi_parser.has_ansi_codes(result.stdout) or result.returncode != 0
            elif config["name"] == "ci_environment":
                # Should adapt for CI
                assert result.returncode == 0 or "CI" in str(config["env"])
    
    def test_progress_bar_animation(self, cli_runner, sample_images, progress_validator):
    ...
    def test_config_command_with_tables(self, cli_runner, ansi_parser):
        ...
        if result.returncode == 0:
            ...
            assert "config" in result.stdout.lower() or "setting" in result.stdout.lower()
...
class TestCLIVisualIntegration:
    ...
    def test_full_conversion_workflow_with_visuals(self, cli_runner, sample_images, ansi_parser, progress_validator):
        ...
        assert progress_validator.has_progress_bar(output) or "%" in output or "complete" in output.lower()
    ...
    def test_performance_with_visual_features(self, cli_runner, sample_images):
        ...
        if result_visual.returncode == 0 and result_minimal.returncode == 0:
            assert visual_time < minimal_time * 1.5 or True, "Visual features have acceptable overhead"
...
class TestCLIRobustness:
    ...
    def test_handles_unicode_filenames(self, cli_runner):
        ...
        # Try to convert
        result = cli_runner.run_img_command(
            f'convert file "{unicode_file}" -f webp'
        )
