...
class PerformanceMetrics:
    ...
    def add_file_metric(self, file_index: int, processing_time: float, 
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "throughput_files_per_second": len(successful_files) / total_time if total_time > 0 else 0,
            "total_output_size_mb": sum(m["output_size"] for m in successful_files) / 1024 / 1024
        }


def create_test_image(size: Tuple[int, int] = (1920, 1080), 
...
class TestBatchPerformance:
    ...
    async def test_medium_batch_performance(self, setup_batch_service):
        ...
        if len(progress_samples) > 5:
            ...
            assert abs(mid_progress - expected_mid_progress) < 20  # Within 20% tolerance
        ...
        assert summary["throughput_files_per_second"] > 0.5  # At least 0.5 files per second
    ...
    async def test_large_batch_performance(self, setup_batch_service):
        ...
        if worker_activity:
            ...
            worker_efficiency = avg_files_per_sample / (max(worker_activity.keys()) / len(worker_activity))
        else:
            worker_efficiency = 0
    ...
    async def test_concurrent_batch_performance(self, setup_batch_service):
        ...
        async def create_and_process_batch(batch_index: int, config: Dict[str, Any]):
            ...
            for i in range(20):
                ...
                files.append(MockUploadFile(f"concurrent_{batch_index}_{i}.jpg", content))
        ...
        logger.info(f"Total time: {total_time:.2f}s, Memory delta: {memory_stats['max_delta_mb']:.2f}MB")
    ...
    async def test_worker_pool_optimization(self, setup_batch_service):
        ...
        logger.info(f"Optimal worker count: {optimal['worker_count']} "
