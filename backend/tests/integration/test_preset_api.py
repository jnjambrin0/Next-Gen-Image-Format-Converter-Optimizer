...
class TestPresetAPI:
    ...
    def test_create_preset(self, client, sample_preset_payload):
        ...
        assert preset["settings"]["output_format"] == sample_preset_payload["settings"]["output_format"]
        assert not preset["is_builtin"]
        assert "id" in preset
    ...
    def test_update_preset_partial(self, client, sample_preset_payload):
        ...
        assert updated["settings"]["output_format"] == preset["settings"]["output_format"]  # Unchanged
    ...
    def test_update_builtin_preset(self, client):
        ...
        list_response = client.get("/api/presets")
        builtin_preset = next(p for p in list_response.json()["presets"] if p["is_builtin"])
        
        # Try to update
        update_payload = {"name": "Cannot Update This"}
        response = client.put(f"/api/presets/{builtin_preset['id']}", json=update_payload)
