...
class TestResourceMonitor:
    ...
    def test_resource_monitor_check_limits(self):
        ...
        limits = ResourceLimits(
            max_memory_mb=10000  # Very high limit
        )
...
class TestDoSPrevention:
    ...
    def test_dos_rapid_modification_prevention(self, dos_watcher, tmp_path):
        ...
        events = [e for e in dos_watcher.processed_files 
                  if "rapid.txt" in str(e)]
    ...
    def test_dos_memory_exhaustion_prevention(self, dos_watcher):
        ...
        with patch.object(dos_watcher.resource_monitor, 'check_limits', return_value=False):
            dos_watcher.start()
            time.sleep(0.1)
            
            # Try to process files
            for i in range(10):
                dos_watcher._process_file_event(
                    Path(f"test{i}.txt"),
                    "created"
                )
    ...
    def test_dos_automatic_shutdown(self, dos_watcher):
        ...
        with patch.object(dos_watcher.resource_monitor, '_monitor_loop') as mock_monitor:
    ...
    def test_dos_path_traversal_prevention(self, dos_watcher, tmp_path):
        ...
        for name in suspicious_names:
            try:
                ...
                safe_name = name.replace("/", "_").replace("\\", "_").replace("\x00", "").replace("\n", "_")
