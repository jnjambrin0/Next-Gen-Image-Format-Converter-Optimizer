...
class TestRecommendationAPI:
    ...
    def test_recommend_endpoint_with_priority(self, client, sample_classification):
        ...
        assert "compression" in str(top_rec["reasons"]).lower() or \
               "smaller" in str(top_rec["reasons"]).lower()
    ...
    async def test_preference_learning_integration(self, client):
        ...
        initial_png_rank = next((i for i, r in enumerate(initial_recs) if r["format"] == OutputFormat.PNG.value), None)
        new_png_rank = next((i for i, r in enumerate(new_recs) if r["format"] == OutputFormat.PNG.value), None)
