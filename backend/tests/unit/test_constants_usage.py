...
class TestConstantsUsage:
    ...
    def test_config_imports_constants(self):
        ...
        assert settings.sandbox_limits_standard["memory_mb"] == SANDBOX_MEMORY_LIMITS["standard"]
        assert settings.sandbox_limits_strict["cpu_percent"] == SANDBOX_CPU_LIMITS["strict"]
        assert settings.sandbox_limits_paranoid["timeout_seconds"] == SANDBOX_TIMEOUTS["paranoid"]
