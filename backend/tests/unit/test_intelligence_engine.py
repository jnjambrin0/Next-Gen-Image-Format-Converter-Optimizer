...
class TestIntelligenceEngine:
    ...
    @pytest.mark.asyncio
    async def test_classify_content_cascade_architecture(self, intelligence_engine, sample_image_bytes):
    ...
    @pytest.mark.asyncio
    async def test_classify_content_caching(self, intelligence_engine, sample_image_bytes):
    ...
    async def test_model_validation_checksum_mismatch(self):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            with patch('pathlib.Path.stat') as mock_stat:
                ...
                with patch('builtins.open', mock_open(read_data='{"checksum": "invalid"}')):
    ...
    @pytest.mark.asyncio
    async def test_secure_memory_clearing(self, intelligence_engine, sample_image_bytes):
        """Test that sensitive data is cleared from cache."""
        # Classify image
        result = await intelligence_engine.classify_content(sample_image_bytes)
        
        # Add some fake sensitive data
        result.face_regions = [BoundingBox(x=10, y=10, width=50, height=50, confidence=0.9)]
