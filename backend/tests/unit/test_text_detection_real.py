...
class TestTextDetectorWithRealImages:
    ...
    def test_detect_code_text(self, text_detector, code_image):
        ...
        similar_height_count = sum(1 for h in heights if abs(h - avg_height) < avg_height * 0.3)
    ...
    def test_confidence_scores_correlation(self, text_detector, fixtures_path):
        ...
        if clear_text.exists() and mixed_text.exists():
            ...
            if clear_regions and mixed_regions:
                ...
                clear_avg_conf = sum(r.confidence for r in clear_regions) / len(clear_regions)
                mixed_avg_conf = sum(r.confidence for r in mixed_regions) / len(mixed_regions)
