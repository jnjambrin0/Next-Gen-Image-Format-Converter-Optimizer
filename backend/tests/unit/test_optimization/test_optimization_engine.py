...
class TestOptimizationEngine:
    ...
    def mock_quality_analyzer(self):
        ...
        analyzer = MagicMock()
        analyzer.calculate_metrics = AsyncMock(return_value={
            'ssim_score': 0.95,
            'psnr_value': 35.0
        })
        return analyzer
    ...
    @pytest.mark.asyncio
    async def test_optimize_balanced_mode(self, engine, test_image, mock_conversion_func):
    ...
    @pytest.mark.asyncio
    async def test_optimize_size_mode_with_target(self, engine, test_image, mock_conversion_func):
        ...
        size_diff_percent = abs(result.final_size - target_size_kb * 1024) / (target_size_kb * 1024) * 100
        assert size_diff_percent <= 10  # Allow some tolerance
    
    @pytest.mark.asyncio
    async def test_optimize_quality_mode(self, engine, test_image, mock_conversion_func):
    ...
    @pytest.mark.asyncio
    async def test_optimize_perceptual_mode(self, engine, test_image, mock_conversion_func):
    ...
    async def test_convergence(self, engine, test_image):
        ...
        async def quick_convergence_func(image_data, output_format, quality=85, **kwargs):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_optimizations(self, engine, test_image, mock_conversion_func):
