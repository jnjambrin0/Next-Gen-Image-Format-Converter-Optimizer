...
class TestQualityAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_calculate_metrics_basic(
        self, analyzer, test_image_rgb, compressed_image
    ):
        """Test basic metric calculation."""
        metrics = await analyzer.calculate_metrics(
            test_image_rgb, compressed_image, calculate_ssim=True, calculate_psnr=True
        )
    ...
    async def test_calculate_metrics_identical_images(self, analyzer, test_image_rgb):
        ...
        metrics = await analyzer.calculate_metrics(
            test_image_rgb, test_image_rgb, calculate_ssim=True, calculate_psnr=True
        )
    ...
    async def test_caching(self, analyzer, test_image_rgb, compressed_image):
        ...
        # First call
        metrics1 = await analyzer.calculate_metrics(test_image_rgb, compressed_image)

        # Second call should hit cache
        metrics2 = await analyzer.calculate_metrics(test_image_rgb, compressed_image)
        ...
        metrics3 = await analyzer.calculate_metrics(test_image_rgb, compressed_image)
    ...
    async def test_large_image_downsampling(self, analyzer):
        ...
        metrics = await analyzer.calculate_metrics(large_data, large_data)

        assert (
            metrics["ssim_score"] > 0.99
        )  # Should still be identical after downsampling
