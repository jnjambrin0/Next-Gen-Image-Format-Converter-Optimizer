...
from app.core.optimization.region_optimizer import (
    RegionOptimizer,
    RegionType,
    Region
)
...
class TestRegionOptimizer:
    ...
    def mock_intelligence_engine(self):
        ...
        classification = ContentClassification(
            ...
            has_faces=True,
            face_regions=[
                BoundingBox(x=20, y=20, width=30, height=40, confidence=0.9)
            ],
            has_text=True,
            text_regions=[
    ...
    @pytest.mark.asyncio
    async def test_optimize_regions_basic(self, optimizer, test_image, mock_conversion_func):
    ...
    def test_merge_overlapping_regions(self, optimizer):
        ...
        regions = [
            ...
            Region(RegionType.TEXT, (20, 20, 60, 60), 0.8, 0.95),  # 44% overlap with face
    ...
    def test_calculate_overlap(self, optimizer):
        ...
        # No overlap
        overlap = optimizer._calculate_overlap(
            (0, 0, 10, 10),
            (20, 20, 30, 30)
        )
        assert overlap == 0.0
    ...
    async def test_quality_factors_application(self, optimizer, test_image):
        ...
        regions = [
            Region(RegionType.FACE, (10, 10, 30, 30), 0.9, optimizer.quality_factors[RegionType.FACE]),
            Region(RegionType.TEXT, (50, 50, 80, 80), 0.85, optimizer.quality_factors[RegionType.TEXT]),
            Region(RegionType.BACKGROUND, (0, 0, 200, 200), 0.7, optimizer.quality_factors[RegionType.BACKGROUND])
        ]
