...
def sample_jpeg_data():
    ...
    return b"\xFF\xD8\xFF\xE0\x00\x10JFIF\x00\x01\x01\x01\x00H\x00H\x00\x00" + b"\x00" * 100
...
class TestFormatDetectionEndpoint:
    ...
    def test_detect_format_file_too_large(self, client):
        ...
        try:
            ...
            assert "body too large" in data["message"].lower() or "size exceeds" in data["message"].lower()
        finally:
    ...
    def test_detect_format_unknown_format(self, mock_service, client, sample_jpeg_data):
        ...
        response = client.post(
            "/api/v1/detection/detect-format",
            files={"file": ("test.unknown", sample_jpeg_data, "application/octet-stream")}
        )
...
class TestFormatRecommendationEndpoint:
    ...
    @patch('app.api.routes.detection.format_detection_service')
    def test_recommend_format_success(self, mock_detection, mock_intelligence, mock_recommendation, client, sample_jpeg_data):
        ...
        mock_recommendation.get_recommendations = AsyncMock(return_value={
            "recommendations": [
                {
                    "format": "webp",
                    "score": 0.9,
    ...
    @patch('app.api.routes.detection.format_detection_service')
    def test_recommend_format_detection_fails(self, mock_detection, client, sample_jpeg_data):
    ...
    @patch('app.api.routes.detection.intelligence_service')
    @patch('app.api.routes.detection.format_detection_service')
    def test_recommend_format_intelligence_fallback(self, mock_detection, mock_intelligence, mock_recommendation, client, sample_jpeg_data):
        """Test recommendation when intelligence service fails (uses fallback)."""
        # Mock services
        mock_detection.detect_format = AsyncMock(return_value=("jpeg", 0.95))
        mock_intelligence.classify_content = AsyncMock(side_effect=Exception("Intelligence service error"))
...
class TestFormatCompatibilityEndpoint:
    ...
    def test_get_format_compatibility_success(self, client):
        ...
        expected_inputs = ["jpeg", "jpg", "png", "webp", "gif", "bmp", "tiff", "heif", "heic", "avif"]
        ...
        for entry in matrix:
            ...
            if entry["input_format"] == "gif":
                ...
                assert any("animation" in limitation.lower() for limitation in entry["limitations"])
...
class TestDetectionEndpointValidation:
    ...
    def test_service_unavailable_under_load(self, mock_semaphore, client, sample_jpeg_data):
...
class TestDetectionEdgeCases:
    ...
    @patch('app.api.routes.detection.intelligence_service')
    @patch('app.api.routes.detection.recommendation_service')
    def test_recommend_format_with_multiple_formats(self, mock_rec, mock_intel, mock_detect, client, sample_gif_data):
        ...
        assert len(data["recommendations"]) == 2
        assert any("animation" in reason.lower() for reason in data["recommendations"][0]["reasons"])
    
    def test_detect_format_with_special_characters_filename(self, client, sample_jpeg_data):
    ...
    def test_compatibility_matrix_completeness(self, client):
        ...
        matrix_inputs = set(entry["input_format"] for entry in data["compatibility_matrix"])
        assert input_formats == matrix_inputs
        
        # Check special format limitations are documented
        gif_entry = next(e for e in data["compatibility_matrix"] if e["input_format"] == "gif")
        assert gif_entry["limitations"] is not None
        assert len(gif_entry["limitations"]) > 0
        
        heif_entry = next(e for e in data["compatibility_matrix"] if e["input_format"] == "heif")
    ...
    def test_recommendation_quality_impact_descriptions(self, client, sample_jpeg_data):
        ...
        with patch('app.api.routes.detection.format_detection_service') as mock_detect:
            with patch('app.api.routes.detection.intelligence_service') as mock_intel:
                with patch('app.api.routes.detection.recommendation_service') as mock_rec:
                    ...
                    mock_rec.get_recommendations = AsyncMock(return_value={
                        "recommendations": [
                            {"format": "webp", "score": 0.9, "reasons": ["Test"], "quality_score": 0.95},
                            {"format": "avif", "score": 0.8, "reasons": ["Test"], "quality_score": 0.85},
                            {"format": "jpeg", "score": 0.7, "reasons": ["Test"], "quality_score": 0.75},
                            {"format": "png", "score": 0.6, "reasons": ["Test"], "quality_score": 0.65},
                            {"format": "jxl", "score": 0.5, "reasons": ["Test"], "quality_score": 0.55},
                        ]
                    })
                    ...
                    quality_impacts = [r["quality_impact"] for r in data["recommendations"]]
