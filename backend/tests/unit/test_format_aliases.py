...
class TestFormatAliases:
    ...
    def test_all_aliases_have_canonical(self):
        ...
        for alias, canonical in FORMAT_ALIASES.items():
            ...
            if canonical.endswith("_opt"):
                continue
            assert canonical in CANONICAL_FORMATS, f"Alias {alias} maps to {canonical} which is not canonical"
    ...
    def test_no_circular_aliases(self):
        ...
        for alias, canonical in FORMAT_ALIASES.items():
            ...
            assert canonical not in FORMAT_ALIASES, f"Circular alias: {alias} -> {canonical}"
