...
class TestDryRunSimulator:
    ...
    def test_estimate_batch_conversion(self, simulator, temp_files):
        ...
        sequential_time = sum(e.estimated_time_seconds for e in batch_estimate.file_estimates)
    ...
    def test_format_specific_estimates(self, simulator, temp_files):
        ...
        assert avif_estimate.estimated_time_seconds > webp_estimate.estimated_time_seconds
    ...
    def test_warning_generation(self, simulator, tmp_path):
        ...
        assert any("time" in w.lower() or "slow" in w.lower() for w in estimate.warnings)
    ...
    def test_preset_modifier_effects(self, simulator, temp_files):
        ...
        assert fast_estimate.estimated_time_seconds < base_estimate.estimated_time_seconds
        
        # Archive should be smaller
        assert archive_estimate.estimated_output_size < base_estimate.estimated_output_size
    ...
    def test_disk_space_validation(self, simulator, temp_files):
        ...
        with patch('shutil.disk_usage') as mock_disk:
            ...
            mock_disk.return_value = MagicMock(
                free=1024 * 1024  # Only 1MB free
            )
            
            validation = simulator.validate_disk_space(
                output_dir=Path("/tmp"),
                estimated_size_mb=100  # Need 100MB
            )
    ...
    def test_memory_estimation_accuracy(self, simulator, temp_files):
        ...
        small_estimate = simulator.estimate_resources(
            input_files=[temp_files[0]],  # ~100KB
            output_format="webp"
        )
