...
from app.cli.utils.terminal import (
    TerminalDetector, TerminalCapability, get_terminal_detector,
    adapt_output, should_use_emoji, should_use_color, get_safe_width
)
...
class TestTerminalDetector:
    ...
    def test_capability_level_standard(self, detector):
        ...
        with patch.object(detector, 'is_ci', return_value=False):
            with patch.object(detector, 'supports_color', return_value=True):
                with patch.object(detector, 'supports_unicode', return_value=True):
                    with patch.object(detector, 'supports_truecolor', return_value=False):
                        assert detector.get_capability_level() == TerminalCapability.STANDARD
    ...
    def test_capability_level_full(self, detector):
        ...
        with patch.object(detector, 'is_ci', return_value=False):
            with patch.object(detector, 'supports_color', return_value=True):
                with patch.object(detector, 'supports_unicode', return_value=True):
                    with patch.object(detector, 'supports_truecolor', return_value=True):
                        assert detector.get_capability_level() == TerminalCapability.FULL
...
class TestTerminalHelpers:
    ...
    def test_adapt_output_minimal(self):
        ...
        with patch.object(detector, 'get_capability_level', return_value=TerminalCapability.MINIMAL):
    ...
    def test_adapt_output_full(self):
        ...
        with patch.object(detector, 'get_capability_level', return_value=TerminalCapability.FULL):
            result = adapt_output("fancy text", "simple text")
