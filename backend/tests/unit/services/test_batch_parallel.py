...
def sample_batch_job():
    ...
    items = [
        BatchItem(file_index=i, filename=f"test_{i}.jpg", status=BatchItemStatus.PENDING)
        for i in range(10)
    ]
...
class TestBatchManagerParallel:
    ...
    @pytest.mark.asyncio
    async def test_parallel_processing_performance(self, batch_manager, sample_batch_job):
        ...
        while sample_batch_job.status != BatchStatus.COMPLETED and time.time() - start_time < max_wait:
    ...
    async def test_concurrent_job_handling(self, batch_manager):
        ...
        for i in range(3):
            job = BatchJob(
                ...
                items=[
                    BatchItem(file_index=j, filename=f"file_{j}.jpg", status=BatchItemStatus.PENDING)
                    for j in range(5)
                ],
                created_at=datetime.utcnow()
            )
    ...
    async def test_semaphore_limits_concurrency(self, batch_manager, sample_batch_job):
        ...
        async def mock_convert(*args, **kwargs):
            ...
            concurrent_count -= 1
            return (
                ConversionResult(success=True, input_format="jpeg", input_size=100000, output_format="webp", output_size=50000, processing_time=0.1),
                b"output"
            )
