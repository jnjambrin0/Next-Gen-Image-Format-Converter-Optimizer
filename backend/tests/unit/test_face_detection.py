...
class TestFaceDetector:
    ...
    def test_calculate_importance_scores(self, face_detector):
        ...
        faces = [
            BoundingBox(x=150, y=150, width=100, height=120, confidence=0.8),  # Center, large
            BoundingBox(x=10, y=10, width=50, height=60, confidence=0.7),      # Corner, small
        ]
    ...
    def test_face_limit(self, face_detector):
        ...
        with patch.object(face_detector, '_non_maximum_suppression', return_value=mock_detections):
            with patch.object(face_detector, '_detect_with_heuristics', return_value=[]):
    ...
    def test_detection_error_handling(self, face_detector):
        ...
        with patch.object(face_detector, '_detect_with_heuristics', side_effect=Exception("Test error")):
            faces = face_detector.detect(Image.new('RGB', (100, 100)))
