...
class TestSecureConnectionManager:
    ...
    def test_check_rate_limit_reset(self, manager):
        ...
        manager._rate_limits[client_ip] = (count, datetime.utcnow() - timedelta(minutes=2))
    ...
    async def test_connect_with_authentication(self, manager):
        ...
        with patch('app.api.websockets.secure_progress.batch_service') as mock_service:
            ...
            with patch('app.api.websockets.secure_progress.settings') as mock_settings:
                ...
                result = await manager.connect(mock_websocket, job_id, token, "192.168.1.100")
    ...
    async def test_connect_invalid_token(self, manager):
        ...
        with patch('app.api.websockets.secure_progress.batch_service') as mock_service:
            ...
            with patch('app.api.websockets.secure_progress.settings') as mock_settings:
                ...
                # Connect should fail
                result = await manager.connect(mock_websocket, job_id, "invalid-token", "192.168.1.100")
    ...
    async def test_connect_invalid_job(self, manager):
        ...
        with patch('app.api.websockets.secure_progress.batch_service') as mock_service:
            ...
            # Connect should fail
            result = await manager.connect(mock_websocket, job_id, None, "192.168.1.100")
            assert result is False
            mock_websocket.close.assert_called_once()
