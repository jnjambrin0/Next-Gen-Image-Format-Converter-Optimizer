...
describe('BatchQueueComponent', () => {
  ...
  describe('item management', () => {
    const mockItems = [
      ...
      { index: 3, filename: 'test4.gif', status: 'failed', progress: 0, error: 'Conversion failed' }
    ]
    ...
    it('should update status with error', () => {
      ...
      const errorMsg = Array.from(allRedTexts).find(el => 
        el.tagName === 'P' && 
        el.className.includes('text-xs') && 
        el.className.includes('mt-2')  // Error messages have mt-2
      )
  ...
  describe('rendering', () => {
    it('should not show cancel button for completed items', () => {
      ...
      const mockItems = [
        { index: 0, filename: 'test1.jpg', status: 'completed' }
      ]
