...
describe('BatchSummaryModal', () => {
  ...
  describe('show/hide', () => {
    const mockResults = {
      ...
      failed_files: [
        { filename: 'test3.gif', error: 'Conversion failed' }
      ],
  ...
  describe('statistics display', () => {
    it('should display correct statistics', () => {
      ...
      const values = Array.from(statCards).map(card => 
        card.querySelector('.text-2xl')?.textContent
      ).filter(Boolean)
    ...
    it('should format long processing times', () => {
      ...
      const timeText = Array.from(document.querySelectorAll('.text-gray-600'))
        .find(el => el.textContent.includes('processing time'))
  ...
  describe('results tables', () => {
    it('should display successful files table', () => {
      ...
      const successTitle = Array.from(document.querySelectorAll('h4'))
        .find(el => el.textContent === 'Successfully Converted')
    ...
    it('should display failed files table', () => {
      ...
      const failedTitle = Array.from(document.querySelectorAll('h4'))
        .find(el => el.textContent === 'Failed Conversions')
  ...
  describe('action buttons', () => {
    it('should show retry button when there are failed files', () => {
      ...
      const retryBtn = Array.from(document.querySelectorAll('button'))
        .find(btn => btn.textContent.includes('Retry Failed Files'))
