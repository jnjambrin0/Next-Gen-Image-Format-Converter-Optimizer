...
class AsyncImageConverterClient:
    ...
    def __init__(
        ...
        if self.verify_localhost and host not in self.ALLOWED_HOSTS:
            raise NetworkSecurityError(
                f"Connection to non-localhost host blocked for security"
            )
        ...
        if not self.api_key:
            self.api_key = self._key_manager.get_from_env() or \
                          self._key_manager.retrieve_api_key("default")
    ...
    async def convert_image(
        ...
        files = {
            "file": (image_path.name, image_data, "application/octet-stream")
        }
